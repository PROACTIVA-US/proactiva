...
describe("agent local JWT", () => {
  const secretEnv = "PROACTIVA_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "PROACTIVA_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "PROACTIVA_AGENT_JWT_ISSUER";
  const audienceEnv = "PROACTIVA_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when PROACTIVA_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "proactiva";
    process.env[audienceEnv] = "proactiva-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
