...
import type { AdapterExecutionContext } from "@proactiva/adapter-utils";
import { ensureProactivaSkillSymlink, type ProactivaSkillEntry } from "@proactiva/adapter-utils/server-utils";

const DEFAULT_PROACTIVA_INSTANCE_ID = "default";

type SkillEntry = ProactivaSkillEntry;
...
function resolveManagedClaudePromptCacheRoot(
  ...
  const proactivaHome = nonEmpty(env.PROACTIVA_HOME) ?? path.resolve(os.homedir(), ".proactiva");
  const instanceId = nonEmpty(env.PROACTIVA_INSTANCE_ID) ?? DEFAULT_PROACTIVA_INSTANCE_ID;
  return path.resolve(
    proactivaHome,
    "instances",
    instanceId,
...
async function buildClaudePromptBundleKey(input: {
  ...
  hash.update("proactiva-claude-prompt-bundle:v1\n");
...
export async function prepareClaudePromptBundle(input: {
  ...
  for (const entry of skills) {
    ...
    try {
      await ensureProactivaSkillSymlink(entry.source, target);
    } catch (err) {
      await onLog(
        "stderr",
        `[proactiva] Failed to materialize Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
