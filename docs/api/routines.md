...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Proactiva-Signature` + `X-Proactiva-Timestamp` header pair matching the trigger's signing mode.
