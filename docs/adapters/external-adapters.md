...
summary: Build, package, and distribute adapters as plugins without modifying Proactiva source
---

Proactiva supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Proactiva's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `proactiva-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Proactiva | Published to npm or linked via `file:` |
| Updates | Requires Proactiva release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@proactiva/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `proactiva.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@proactiva/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Proactiva vars injected
  const env = buildProactivaEnv(agent);
...
#### Available Helpers from `@proactiva/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildProactivaEnv(agent)` | Inject `PROACTIVA_*` environment variables |
...
```sh
# Via the Proactiva UI
# Settings → Adapters → Install from npm → "my-proactiva-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-proactiva-adapter"}'
```
...
Local adapters are symlinked into Proactiva's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.proactiva/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-proactiva-adapter",
...
```ts
import type { AdapterSessionCodec } from "@proactiva/adapter-utils";
...
Other Proactiva users can then install your adapter by package name from the UI or API.
