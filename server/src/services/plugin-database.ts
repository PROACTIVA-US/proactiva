...
export function pluginDatabaseService(db: Db) {
  ...
  async function ensureNamespace(pluginId: string, manifest: ProactivaPluginManifestV1) {
  ...
  return {
    ...
    async applyMigrations(pluginId: string, manifest: ProactivaPluginManifestV1, packageRoot: string) {
      if (!manifest.database) return null;
