...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`proactiva-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="proactiva-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("proactiva-mermaid-status", error && "proactiva-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="proactiva-mermaid-source">
...
export function MarkdownBody({
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `proactiva-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "proactiva-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "proactiva-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
