...
describe("cli auth routes", () => {
  ...
  it("creates a CLI auth challenge with approval metadata", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        command: "proactiva company import",
        clientName: "proactiva cli",
  ...
  it("rejects anonymous access to generic skill documents", async () => {
    ...
    const [indexRes, skillRes] = await Promise.all([
      ...
      request(app).get("/api/skills/proactiva"),
    ]);
  ...
  it("serves the invite-scoped proactiva skill anonymously for active invites", async () => {
    const invite = {
      id: "invite-1",
      companyId: "company-1",
    ...
    const res = await request(app).get("/api/invites/token-123/skills/proactiva");

    expect(res.status).toBe(200);
    expect(res.headers["content-type"]).toContain("text/markdown");
    expect(res.text).toContain("# Proactiva Skill");
  });
