...
import { execute } from "@proactiva/adapter-codex-local/server";

async function writeFakeCodexCommand(commandPath: string): Promise<void> {
  const script = `#!/usr/bin/env node
const fs = require("node:fs");

const capturePath = process.env.PROACTIVA_TEST_CAPTURE_PATH;
const payload = {
  argv: process.argv.slice(2),
  prompt: fs.readFileSync(0, "utf8"),
  codexHome: process.env.CODEX_HOME || null,
  proactivaWakePayloadJson: process.env.PROACTIVA_WAKE_PAYLOAD_JSON || null,
  proactivaEnvKeys: Object.keys(process.env)
    .filter((key) => key.startsWith("PROACTIVA_"))
    .sort(),
};
if (capturePath) {
...
type CapturePayload = {
  ...
  proactivaWakePayloadJson: string | null;
  proactivaEnvKeys: string[];
};
...
describe("codex execute", () => {
  it("uses a Proactiva-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-default-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const proactivaHome = path.join(root, "proactiva-home");
    ...
    const previousProactivaHome = process.env.PROACTIVA_HOME;
    const previousProactivaInstanceId = process.env.PROACTIVA_INSTANCE_ID;
    const previousProactivaInWorktree = process.env.PROACTIVA_IN_WORKTREE;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.PROACTIVA_HOME = proactivaHome;
    delete process.env.PROACTIVA_INSTANCE_ID;
    delete process.env.PROACTIVA_IN_WORKTREE;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            PROACTIVA_TEST_CAPTURE_PATH: capturePath,
          },
          promptTemplate: "Follow the proactiva heartbeat.",
        },
        context: {},
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using Proactiva-managed Codex home"),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousProactivaHome === undefined) delete process.env.PROACTIVA_HOME;
      else process.env.PROACTIVA_HOME = previousProactivaHome;
      if (previousProactivaInstanceId === undefined) delete process.env.PROACTIVA_INSTANCE_ID;
      else process.env.PROACTIVA_INSTANCE_ID = previousProactivaInstanceId;
      if (previousProactivaInWorktree === undefined) delete process.env.PROACTIVA_IN_WORKTREE;
      else process.env.PROACTIVA_IN_WORKTREE = previousProactivaInWorktree;
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-notes-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-meta-"));
    const workspace = path.join(root, "workspace");
    ...
    try {
      ...
      expect(loggedEnv.PROACTIVA_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("injects structured Proactiva wake payloads into env and prompt", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.proactivaEnvKeys).toContain("PROACTIVA_WAKE_PAYLOAD_JSON");
      expect(capture.proactivaWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.proactivaWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## Proactiva Wake Payload");
  ...
  it("classifies remote-compaction high-demand failures as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses safer invocation settings and a fresh-session handoff for codex transient fallback retries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-fallback-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.prompt).toContain("Proactiva session handoff:");
  ...
  it("renders execution-stage wake instructions for reviewer and executor roles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-stage-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const executorResult = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            PROACTIVA_TEST_CAPTURE_PATH: executorCapturePath,
          },
          promptTemplate: "Follow the proactiva heartbeat.",
        },
        context: {
  ...
  it("renders an issue-scoped wake prompt even when the wake has no comments yet", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-issue-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    ...
    try {
      ...
      expect(capture.prompt).toContain("## Proactiva Resume Delta");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      expect(capture.prompt).not.toContain("Follow the proactiva heartbeat.");
      expect(capture.prompt).not.toContain("You are managed instructions.");
      expect(invocationPrompt).toContain("## Proactiva Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const homeSkill = path.join(isolatedCodexHome, "skills", "proactiva");
    await fs.mkdir(workspace, { recursive: true });
    await fs.mkdir(sharedCodexHome, { recursive: true });
    await fs.writeFile(path.join(sharedCodexHome, "auth.json"), '{"token":"shared"}\n', "utf8");
    await fs.writeFile(path.join(sharedCodexHome, "config.toml"), 'model = "codex-mini-latest"\n', "utf8");
    ...
    process.env.HOME = root;
    process.env.PROACTIVA_HOME = proactivaHome;
    process.env.PROACTIVA_INSTANCE_ID = "worktree-1";
    process.env.PROACTIVA_IN_WORKTREE = "true";
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      expect(capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      expect(capture.prompt).toContain("Follow the proactiva heartbeat.");
      expect(capture.proactivaEnvKeys).toEqual(
        expect.arrayContaining([
          "PROACTIVA_AGENT_ID",
          "PROACTIVA_API_KEY",
          "PROACTIVA_API_URL",
          "PROACTIVA_COMPANY_ID",
          "PROACTIVA_RUN_ID",
        ]),
      );
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining('Injected Codex skill "proactiva"'),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-codex-execute-explicit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      ...
      expect((await fs.lstat(path.join(explicitCodexHome, "skills", "proactiva"))).isSymbolicLink()).toBe(true);
      await expect(fs.lstat(path.join(proactivaHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousProactivaHome === undefined) delete process.env.PROACTIVA_HOME;
