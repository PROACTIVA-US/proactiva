...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@proactiva/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "proactiva" &&
    normalized !== "proactiva-create-agent" &&
    normalized !== "proactiva-create-plugin" &&
...
/** Resolve the Proactiva repo skills directory (built-in / managed skills). */
function resolveProactivaSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const proactivaSkillsDir = resolveProactivaSkillsDir();

  // Build set of Proactiva-managed skill names
  const proactivaSkillNames = new Set<string>();
  if (proactivaSkillsDir) {
    try {
      for (const entry of fs.readdirSync(proactivaSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) proactivaSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isProactivaManaged: proactivaSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.proactivaApiUrl)) {
    const legacyProactivaApiUrl = nonEmptyTrimmedString(input.proactivaApiUrl);
    if (legacyProactivaApiUrl) merged.proactivaApiUrl = legacyProactivaApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    proactivaApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.proactivaApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawProactivaApiUrl =
    typeof defaults.proactivaApiUrl === "string"
      ? defaults.proactivaApiUrl.trim()
      : "";
  if (rawProactivaApiUrl) {
    try {
      const parsedProactivaApiUrl = new URL(rawProactivaApiUrl);
      if (
        parsedProactivaApiUrl.protocol !== "http:" &&
        parsedProactivaApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_proactiva_api_url_protocol",
          level: "warn",
          message: `proactivaApiUrl must use http:// or https:// (got ${parsedProactivaApiUrl.protocol}).`
        });
      } else {
        normalized.proactivaApiUrl = parsedProactivaApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_proactiva_api_url_configured",
          level: "info",
          message: `proactivaApiUrl set to ${parsedProactivaApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_proactiva_api_url_invalid",
        level: "warn",
        message: `Invalid proactivaApiUrl: ${rawProactivaApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Proactiva host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Proactiva is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm proactiva allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/proactiva`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/proactiva-claimed-api-key.json and load PROACTIVA_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: proactivaApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Proactiva hostname is reachable and allowed via `pnpm proactiva allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Proactiva API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/proactiva/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Proactiva OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          proactivaApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Proactiva generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "proactivaApiUrl": "https://proactiva-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Proactiva before key claim is allowed.
    ...
    - ~/.openclaw/workspace/proactiva-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/proactiva-claimed-api-key.json

    And set the PROACTIVA_API_KEY and PROACTIVA_API_URL in your environment variables as specified here:
    ...
    Then set PROACTIVA_API_KEY and PROACTIVA_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Proactiva skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your PROACTIVA_API_URL to the top of your skill and note the path to your PROACTIVA_API_URL
    ...
    ${
      ...
      "Ensure Proactiva is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Proactiva base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.proactivaApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm proactiva allowed-hostname <host>
      - then restart Proactiva and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@proactiva.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "proactiva", path: "/api/skills/proactiva" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "proactiva-create-agent",
          path: "/api/skills/proactiva-create-agent"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "proactiva",
          path: `/api/invites/${token}/skills/proactiva`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "proactiva") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              proactivaApiUrl: req.body.proactivaApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.proactivaApiUrl &&
          !persistedDefaults.proactivaApiUrl
        ) {
          missingPersistedFields.push("proactivaApiUrl");
        }
