# Proactiva Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Proactiva's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Proactiva app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Proactiva HTTP APIs directly.
...
Proactiva plugin design is based on the following assumptions:

1. Proactiva is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Proactiva business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Proactiva core.
...
2. Let plugins add major capabilities without editing Proactiva core.
...
4. Depend on project-local plugin folders such as `.proactiva/plugins`.
...
The single Proactiva deployment an operator installs and controls.

### 5.2 Company

A first-class Proactiva business object inside the instance.
...
A trusted in-process extension loaded directly by Proactiva core.
...
An installable instance-wide extension package loaded through the Proactiva plugin runtime.
...
Proactiva has two extension classes.
...
Proactiva already has a concrete workspace model:
...
Plugins live under the Proactiva instance directory.

Suggested layout:

- `~/.proactiva/instances/default/plugins/package.json`
- `~/.proactiva/instances/default/plugins/node_modules/`
- `~/.proactiva/instances/default/plugins/.cache/`
- `~/.proactiva/instances/default/data/plugins/<plugin-id>/`
...
Proactiva should add CLI commands:

- `pnpm proactiva plugin list`
- `pnpm proactiva plugin install <package[@version]>`
- `pnpm proactiva plugin uninstall <plugin-id>`
- `pnpm proactiva plugin upgrade <plugin-id> [version]`
- `pnpm proactiva plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@proactiva/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface ProactivaPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumProactivaVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Proactiva agents can use during runs.
...
- Proactiva server starts one worker process per installed plugin
...
Receives one typed Proactiva domain event.
...
export function definePlugin(definition: PluginDefinition): ProactivaPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: ProactivaPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@proactiva/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@proactiva/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@proactiva/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@proactiva/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@proactiva/plugin-sdk/ui`
...
The `@proactiva/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Proactiva secret provider system rather than a plain text input
...
1. Core Proactiva data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Proactiva objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Proactiva product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Proactiva later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Proactiva secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm proactiva plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Proactiva server. This is a normative requirement, not optional.
...
### 27.1 `@proactiva/plugin-test-harness`
...
```ts
import { createTestHarness } from "@proactiva/plugin-test-harness";
...
For developing a plugin against a running Proactiva instance:

- The operator installs the plugin from a local path: `pnpm proactiva plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-proactiva-plugin`) that scaffolds:

- `package.json` with correct `proactivaPlugin` keys
...
- `@proactiva/plugin-workspace-files`
- `@proactiva/plugin-terminal`
- `@proactiva/plugin-git`
- `@proactiva/plugin-linear`
- `@proactiva/plugin-github-issues`
- `@proactiva/plugin-grafana`
- `@proactiva/plugin-runtime-processes`
- `@proactiva/plugin-stripe`
...
- `@proactiva/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@proactiva/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@proactiva/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@proactiva/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@proactiva/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@proactiva/plugin-sdk/ui`
...
- `@proactiva/plugin-test-harness`
- `create-proactiva-plugin` starter template
...
Proactiva should not implement a generic in-process hook bag modeled directly after local coding tools.

Proactiva should implement:
...
That is the complete target design for the Proactiva plugin system.
