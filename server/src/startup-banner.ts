...
import { resolveProactivaConfigPath, resolveProactivaEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@proactiva/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.PROACTIVA_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.PROACTIVA_AGENT_JWT_SECRET === "string" ? parsed.PROACTIVA_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm proactiva onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveProactivaEnvPath();
