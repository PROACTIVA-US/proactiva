...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Proactiva Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@proactiva.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("proactiva-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `proactiva-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "proactiva-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "proactiva-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "proactiva-close-check",
      baseRef: "main",
      metadata: {
