...
const JWT_SECRET_ENV_KEY = "PROACTIVA_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Proactiva CLI commands",
...
export function resolveProactivaEnvFile(configPath?: string): string {
...
export function loadProactivaEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeProactivaEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readProactivaEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeProactivaEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeProactivaEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readProactivaEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeProactivaEnvEntries(next, filePath);
  return next;
}
