...
ClipHub is the public registry where people share, discover, and download Proactiva company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Proactiva what a package registry is to a programming language. Proactiva already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Proactiva — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Proactiva instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Proactiva spec. It contains:
...
**Install (fresh start):**
```
proactiva install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Proactiva instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Proactiva, export your company as a template, then publish:

```
proactiva export --template my-company
proactiva publish cliphub my-company
```
...
ClipHub is a **separate service** from Proactiva itself. Proactiva is self-hosted; ClipHub is a hosted registry that Proactiva instances talk to.
...
| **Proactiva CLI** | `proactiva install`, `proactiva publish`, `proactiva cliphub sync` — built into Proactiva |
| **Proactiva UI** | "Browse ClipHub" panel in the Proactiva web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Proactiva) |
| Backend | TypeScript + Hono (consistent with Proactiva) |
| Database | PostgreSQL |
...
4. Run `proactiva install cliphub:acme/lean-saas-shop`
5. Proactiva creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Proactiva until it works well
2. Export: `proactiva export --template my-agency`
3. Publish: `proactiva publish cliphub my-agency`
...
3. Install just that agent: `proactiva install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Proactiva

ClipHub is **not required** to use Proactiva. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Proactiva what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `proactiva install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Proactiva web UI
- [ ] `proactiva cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Proactiva's own philosophy)
