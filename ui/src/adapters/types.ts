...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@proactiva/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@proactiva/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@proactiva/adapter-utils").TranscriptEntry[];
