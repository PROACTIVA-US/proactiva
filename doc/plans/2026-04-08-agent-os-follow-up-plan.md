...
Turn the `agent-os` research into a low-risk Proactiva execution plan that preserves Proactiva's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Proactiva should not absorb `agent-os` as a product model or orchestration layer.

Proactiva should evaluate `agent-os` in three narrow areas:
...
This work is successful when Proactiva has:
...
- a proposed runtime capability model that fits current Proactiva adapters
...
- replace Proactiva heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Proactiva core

## Existing Proactiva integration points
...
- prove that Proactiva can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Proactiva's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Proactiva-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Proactiva workloads
...
- identify where Proactiva should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Proactiva
...
- runtime experimentation could blur the boundary between Proactiva as control plane and Proactiva as execution platform
