...
vi.mock("@proactiva/shared/telemetry", () => ({
...
function registerModuleMocks() {
  vi.doMock("@proactiva/shared/telemetry", () => ({
    trackAgentCreated: mockTrackAgentCreated,
    trackErrorHandlerCrash: vi.fn(),
  }));
...
describe("agent skill routes", () => {
  beforeEach(() => {
    ...
    mockAdapter.listSkills.mockResolvedValue({
      ...
      supported: true,
      mode: "ephemeral",
      desiredSkills: ["proactiva/proactiva/proactiva"],
      entries: [],
      warnings: [],
    });
    ...
    expect(mockAdapter.listSkills).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        config: expect.objectContaining({
          proactivaRuntimeSkills: expect.any(Array),
        }),
      }),
    );
  }, 10_000);
  ...
  it("skips runtime materialization when syncing Claude skills", async () => {
    ...
    const res = await request(await createApp())
      ...
      .send({ desiredSkills: ["proactiva/proactiva/proactiva"] });
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          proactivaSkillSync: expect.objectContaining({
            desiredSkills: ["proactiva/proactiva/proactiva"],
          }),
        }),
      }),
