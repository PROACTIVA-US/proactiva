...
import type { AdapterModel } from "@proactiva/adapter-utils";
import { asString, runChildProcess } from "@proactiva/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.PROACTIVA_PI_COMMAND === "string" &&
    process.env.PROACTIVA_PI_COMMAND.trim().length > 0
      ? process.env.PROACTIVA_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["PROACTIVA_", "npm_", "NPM_"] as const;
