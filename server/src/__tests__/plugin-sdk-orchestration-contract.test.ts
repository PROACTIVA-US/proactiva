...
import type { Issue, ProactivaPluginManifestV1 } from "@proactiva/shared";
import { createTestHarness } from "../../../packages/plugins/sdk/src/testing.js";

function manifest(capabilities: ProactivaPluginManifestV1["capabilities"]): ProactivaPluginManifestV1 {
  return {
    id: "proactiva.test-orchestration",
    apiVersion: 1,
    version: "0.1.0",
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:proactiva.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:proactiva.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:proactiva.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:proactiva.test-orchestration");
    await expect(
