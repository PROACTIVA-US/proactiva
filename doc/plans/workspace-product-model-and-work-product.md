...
Proactiva needs to support two very different but equally valid ways of working:
...
Today, Proactiva already has the beginnings of this model:
...
- making Proactiva feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Proactiva users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Proactiva deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Proactiva into a full code review product
...
This object must be recorded explicitly so that Proactiva can:
...
Proactiva should treat PRs as a type of work product linked back to:
...
Proactiva already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Proactiva must be a first-class deployment mode

Proactiva cannot assume that it is running on the same machine as the code.

In cloud deployments, Proactiva may:
...
- `Runtime service`: a process or service Proactiva owns or tracks for a workspace
...
It also lets cloud-hosted Proactiva deployments point at codebases and remotes without pretending the Proactiva host has direct filesystem access.
...
Without an explicit `execution workspace` record, Proactiva has nowhere stable to attach:
...
Proactiva needs a single place to show:
...
- Proactiva-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Proactiva owning the execution host
...
## 1. Proactiva host and execution host must be decoupled
...
without the Proactiva host itself running local git or local preview processes.
...
Proactiva should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Proactiva.
...
Proactiva should keep workspace automation centered on a small set of hooks:
...
Proactiva should provide the equivalent visibility in the project `Code` surface:
...
Proactiva should treat execution workspaces as the natural home for:
...
Proactiva should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Proactiva should continue treating cleanup policy as part of the core workspace model:
...
Proactiva should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Proactiva host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Proactiva should borrow Worktrunk's good execution patterns while keeping the broader Proactiva model:
...
This is how Proactiva supports:
...
That keeps Proactiva focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Proactiva deployments remain first-class

That is a better fit for Proactiva than either extreme:
