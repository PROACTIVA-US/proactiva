...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Proactiva plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Proactiva.
...
- package: `@proactiva/plugin-kitchen-sink-example`
- plugin id: `proactiva.kitchen-sink-example` or `proactiva-kitchen-sink-example`

Recommendation: use `proactiva-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Proactiva event such as `issue.created`
...
Recommendation: default to a Proactiva-local endpoint or a stable public echo endpoint to avoid flaky docs.
