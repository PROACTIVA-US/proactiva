...
import { getAdapterSessionManagement } from "@proactiva/adapter-utils";
import {
...
} from "@proactiva/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@proactiva/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@proactiva/adapter-codex-local";
import {
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@proactiva/adapter-cursor-local";
import {
...
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@proactiva/adapter-gemini-local";
import {
  ...
  listOpenCodeModels,
} from "@proactiva/adapter-opencode-local/server";
import {
  agentConfigurationDoc as openCodeAgentConfigurationDoc,
  models as openCodeModels,
} from "@proactiva/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@proactiva/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@proactiva/adapter-openclaw-gateway";
...
import {
  ...
  listPiModels,
} from "@proactiva/adapter-pi-local/server";
import {
  agentConfigurationDoc as piAgentConfigurationDoc,
} from "@proactiva/adapter-pi-local";
import {
...
// hermes-proactiva-adapter v0.2.0 predates the authToken field; cast is
...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const explicitApiKey =
      typeof existingEnv.PROACTIVA_API_KEY === "string" && existingEnv.PROACTIVA_API_KEY.trim().length > 0;
    ...
    const authGuardPrompt = [
      "Proactiva API safety rule:",
      "Use Authorization: Bearer $PROACTIVA_API_KEY on every Proactiva API request.",
      "Use X-Proactiva-Run-Id: $PROACTIVA_RUN_ID on every Proactiva API request that writes or mutates data, including comments and issue updates.",
      "Never use a board, browser, or local-board session for Proactiva API writes.",
    ].join("\n");
    ...
    const patchedConfig: Record<string, unknown> = {
      ...
      env: {
        ...existingEnv,
        ...(!explicitApiKey ? { PROACTIVA_API_KEY: normalizedCtx.authToken } : {}),
        PROACTIVA_RUN_ID: normalizedCtx.runId,
      },
    };
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[proactiva] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[proactiva] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[proactiva] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[proactiva] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
