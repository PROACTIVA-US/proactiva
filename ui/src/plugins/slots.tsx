 ...
 *
 * Plugin bundles are built with `external: ["@proactiva/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__proactivaPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__proactivaPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__proactivaPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__proactivaPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@proactiva/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@proactiva/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@proactiva/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@proactiva/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@proactiva/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__proactivaPluginBridge__) {
 ...
 * self-register via `window.proactivaPlugins.registerReactComponent()`.
 ...
 * of `@proactiva/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
