...
const DEFAULT_AGENT_JWT_ISSUER = "proactiva";
const DEFAULT_AGENT_JWT_AUDIENCE = "proactiva-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveProactivaInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveProactivaInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" proactiva env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: ProactivaConfig | null = null;
...
function collectDeploymentEnvRows(config: ProactivaConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrlSource: EnvSource =
    process.env.PROACTIVA_PUBLIC_URL
      ? "env"
      : process.env.PROACTIVA_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const secretsProvider =
    process.env.PROACTIVA_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.PROACTIVA_SECRETS_STRICT_MODE ??
    String(config?.secrets?.strictMode ?? false);
  const secretsKeyFilePath =
    process.env.PROACTIVA_SECRETS_MASTER_KEY_FILE ??
    config?.secrets?.localEncrypted?.keyFilePath ??
    defaultSecretsKeyFilePath();
  const storageProvider =
    process.env.PROACTIVA_STORAGE_PROVIDER ??
    config?.storage?.provider ??
    DEFAULT_STORAGE_PROVIDER;
  const storageLocalDir =
    process.env.PROACTIVA_STORAGE_LOCAL_DIR ??
    config?.storage?.localDisk?.baseDir ??
    defaultStorageBaseDir();
  const storageS3Bucket =
    process.env.PROACTIVA_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "proactiva";
  const storageS3Region =
    process.env.PROACTIVA_STORAGE_S3_REGION ??
    config?.storage?.s3?.region ??
    "us-east-1";
  const storageS3Endpoint =
    process.env.PROACTIVA_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.PROACTIVA_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.PROACTIVA_STORAGE_S3_FORCE_PATH_STYLE ??
    String(config?.storage?.s3?.forcePathStyle ?? false);

  const rows: EnvVarRow[] = [
    {
      key: "PROACTIVA_AGENT_JWT_SECRET",
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from PROACTIVA_PUBLIC_URL when possible)",
    },
    {
      key: "PROACTIVA_AGENT_JWT_TTL_SECONDS",
      value: process.env.PROACTIVA_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.PROACTIVA_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "PROACTIVA_AGENT_JWT_ISSUER",
      value: process.env.PROACTIVA_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.PROACTIVA_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "PROACTIVA_AGENT_JWT_AUDIENCE",
      value: process.env.PROACTIVA_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.PROACTIVA_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "PROACTIVA_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.PROACTIVA_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "PROACTIVA_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.PROACTIVA_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "PROACTIVA_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.PROACTIVA_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "PROACTIVA_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.PROACTIVA_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "PROACTIVA_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.PROACTIVA_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "PROACTIVA_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.PROACTIVA_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "PROACTIVA_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.PROACTIVA_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "PROACTIVA_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.PROACTIVA_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "PROACTIVA_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.PROACTIVA_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "PROACTIVA_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.PROACTIVA_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.PROACTIVA_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "PROACTIVA_CONFIG",
      value: process.env.PROACTIVA_CONFIG ?? configPath,
      source: process.env.PROACTIVA_CONFIG ? "env" : "default",
      required: false,
