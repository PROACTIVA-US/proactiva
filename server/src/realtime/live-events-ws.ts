...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@proactiva/db";
import type { DeploymentMode } from "@proactiva/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  proactivaUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).proactivaUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.proactivaUpgradeContext = context;
