...
import type { AdapterRuntimeServiceReport } from "@proactiva/adapter-utils";
import type { Db } from "@proactiva/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@proactiva/db";
import {
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".proactiva", "dist", "node_modules"]);
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.PROACTIVA_WORKSPACE_CWD = input.worktreePath;
  env.PROACTIVA_WORKSPACE_PATH = input.worktreePath;
  env.PROACTIVA_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.PROACTIVA_WORKSPACE_BRANCH = input.branchName;
  env.PROACTIVA_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.PROACTIVA_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.PROACTIVA_WORKSPACE_SOURCE = input.base.source;
  env.PROACTIVA_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.PROACTIVA_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.PROACTIVA_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.PROACTIVA_PROJECT_ID = input.base.projectId ?? "";
  env.PROACTIVA_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.PROACTIVA_AGENT_ID = input.agent.id ?? "";
  env.PROACTIVA_AGENT_NAME = input.agent.name;
  env.PROACTIVA_COMPANY_ID = input.agent.companyId;
  env.PROACTIVA_ISSUE_ID = input.issue?.id ?? "";
  env.PROACTIVA_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.PROACTIVA_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.PROACTIVA_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.PROACTIVA_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.PROACTIVA_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.PROACTIVA_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.PROACTIVA_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.PROACTIVA_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.PROACTIVA_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.PROACTIVA_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.PROACTIVA_PROJECT_ID = input.workspace.projectId ?? "";
  env.PROACTIVA_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.PROACTIVA_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".proactiva", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Proactiva", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
