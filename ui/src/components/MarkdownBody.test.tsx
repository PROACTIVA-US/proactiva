...
describe("MarkdownBody", () => {
  ...
  it("renders user, agent, project, and skill mentions as chips", () => {
    const html = renderToStaticMarkup(
      <QueryClientProvider client={new QueryClient()}>
        <ThemeProvider>
          <MarkdownBody>
            {`[@Taylor](${buildUserMentionHref("user-123")}) [@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@Proactiva App](${buildProjectMentionHref("project-456", "#336699")}) [/release-changelog](${buildSkillMentionHref("skill-789", "release-changelog")})`}
          </MarkdownBody>
        </ThemeProvider>
      </QueryClientProvider>,
    );
    ...
    expect(html).toContain("--proactiva-mention-icon-mask");
    expect(html).toContain('href="/projects/project-456"');
    expect(html).toContain('data-mention-kind="project"');
    expect(html).toContain("--proactiva-mention-project-color:#336699");
  ...
  it("linkifies bare issue identifiers in markdown text", () => {
    ...
    expect(html).not.toContain("proactiva-mention-chip--issue");
  });
  ...
  it("applies wrap-friendly styles to long inline content", () => {
    ...
    expect(html).toContain('class="proactiva-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden');
  ...
  it("opens GitHub links in a new tab", () => {
    const html = renderMarkdown("[pr](https://github.com/PROACTIVA-US/proactiva/pull/4099)");
  ...
  it("prefixes GitHub markdown links with the GitHub icon", () => {
    const html = renderMarkdown("[https://github.com/PROACTIVA-US/proactiva/pull/4099](https://github.com/PROACTIVA-US/proactiva/pull/4099)");

    expect(html).toContain('<a href="https://github.com/PROACTIVA-US/proactiva/pull/4099"');
    expect(html).toContain('class="lucide lucide-github mr-1 inline h-3.5 w-3.5 align-[-0.125em]"');
    expect(html).toContain(">https://github.com/PROACTIVA-US/proactiva/pull/4099</a>");
  });

  it("prefixes GitHub autolinks with the GitHub icon", () => {
    const html = renderMarkdown("See https://github.com/PROACTIVA-US/proactiva/issues/1778");

    expect(html).toContain('<a href="https://github.com/PROACTIVA-US/proactiva/issues/1778"');
    expect(html).toContain('class="lucide lucide-github mr-1 inline h-3.5 w-3.5 align-[-0.125em]"');
  });
