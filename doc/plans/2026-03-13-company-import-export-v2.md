...
This document defines the next-stage plan for Proactiva company import/export.

The core shift is:

- move from a Proactiva-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Proactiva.
...
Proactiva already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Proactiva
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `proactiva.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.proactiva.yaml` sidecar for high-fidelity Proactiva-specific details
...
11. adapter-aware skill sync surfaces so Proactiva can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Proactiva at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Proactiva sidecar.
...
Proactiva must not redefine `SKILL.md`.
...
- Proactiva-specific fidelity lives in `.proactiva.yaml`
- Proactiva may resolve and install `SKILL.md` packages, but it must not require a Proactiva-only skill format
...
### 5.4 Base Package Vs Proactiva Extension
...
- base package:
  ...
  - no Proactiva-only runtime fields by default
- Proactiva extension:
  - `.proactiva.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Proactiva tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`proactiva.manifest.json` is not part of the future package direction.
...
Proactiva import/export should support these entity kinds:
...
- Proactiva can treat “this set of agents came from team package X” as the imported-team model
...
Proactiva should support source references in package metadata with:
...
Proactiva should define a new adapter surface area around skills:
...
Planned Proactiva behavior:

- if an adapter supports read, Proactiva should show current skills in the UI
- if an adapter supports write, Proactiva should let the user enable/disable imported skills
- if an adapter supports sync, Proactiva should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.proactiva.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.proactiva.yaml` alongside the base package
...
## 10. Storage Model Inside Proactiva
...
Proactiva should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `proactiva company export <company-id> --out <path>`
- `proactiva company import <path-or-url> --dry-run`
- `proactiva company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Proactiva storage, or only referenced at import time?
...
3. Should Proactiva support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Proactiva aligned with:
