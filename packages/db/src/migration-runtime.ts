...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://proactiva:proactiva@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "proactiva");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://proactiva:proactiva@127.0.0.1:${preferredPort}/proactiva`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://proactiva:proactiva@127.0.0.1:${port}/proactiva`,
  ...
  const adminConnectionString = `postgres://proactiva:proactiva@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "proactiva");

  return {
    connectionString: `postgres://proactiva:proactiva@127.0.0.1:${selectedPort}/proactiva`,
