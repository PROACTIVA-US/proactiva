...
import type { Agent } from "@proactiva/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveProactivaSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export PROACTIVA_API_URL='${escaped(input.apiBase)}'`,
    `export PROACTIVA_COMPANY_ID='${escaped(input.companyId)}'`,
    `export PROACTIVA_AGENT_ID='${escaped(input.agentId)}'`,
    `export PROACTIVA_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Proactiva skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Proactiva skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveProactivaSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Proactiva skills directory. Expected ./skills in the repo checkout.",
              );
            }
