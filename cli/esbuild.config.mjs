/**
 * esbuild configuration for building the proactiva CLI for npm.
 *
 * Bundles all workspace packages (@proactiva/*) into a single file.
...
// to npm and resolved at runtime (e.g. @proactiva/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@proactiva/")) {
      externals.add(name);
    }
  }
