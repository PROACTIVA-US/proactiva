...
type OnboardDefaults = Pick<ProactivaConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or PROACTIVA_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "PROACTIVA_PUBLIC_URL",
  "DATABASE_URL",
  "PROACTIVA_DB_BACKUP_ENABLED",
  "PROACTIVA_DB_BACKUP_INTERVAL_MINUTES",
  "PROACTIVA_DB_BACKUP_RETENTION_DAYS",
  "PROACTIVA_DB_BACKUP_DIR",
  "PROACTIVA_DEPLOYMENT_MODE",
  "PROACTIVA_DEPLOYMENT_EXPOSURE",
  "PROACTIVA_BIND",
  "PROACTIVA_BIND_HOST",
  "PROACTIVA_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "PROACTIVA_ALLOWED_HOSTNAMES",
  "PROACTIVA_AUTH_BASE_URL_MODE",
  "PROACTIVA_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "PROACTIVA_STORAGE_PROVIDER",
  "PROACTIVA_STORAGE_LOCAL_DIR",
  "PROACTIVA_STORAGE_S3_BUCKET",
  "PROACTIVA_STORAGE_S3_REGION",
  "PROACTIVA_STORAGE_S3_ENDPOINT",
  "PROACTIVA_STORAGE_S3_PREFIX",
  "PROACTIVA_STORAGE_S3_FORCE_PATH_STYLE",
  "PROACTIVA_SECRETS_PROVIDER",
  "PROACTIVA_SECRETS_STRICT_MODE",
  "PROACTIVA_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<ProactivaConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const instanceId = resolveProactivaInstanceId();
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.PROACTIVA_PUBLIC_URL?.trim() ||
      process.env.PROACTIVA_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.PROACTIVA_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.PROACTIVA_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.PROACTIVA_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.PROACTIVA_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.PROACTIVA_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.PROACTIVA_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.PROACTIVA_ALLOWED_HOSTNAMES
    ? process.env.PROACTIVA_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.PROACTIVA_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.PROACTIVA_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.PROACTIVA_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.PROACTIVA_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.PROACTIVA_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.PROACTIVA_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.PROACTIVA_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.PROACTIVA_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.PROACTIVA_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.PROACTIVA_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.PROACTIVA_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.PROACTIVA_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.PROACTIVA_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.PROACTIVA_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.PROACTIVA_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "PROACTIVA_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.PROACTIVA_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "PROACTIVA_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.PROACTIVA_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "PROACTIVA_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<ProactivaConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" proactiva onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveProactivaInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: ProactivaConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Proactiva install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("proactiva configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("PROACTIVA_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.PROACTIVA_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("PROACTIVA_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("PROACTIVA_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: PROACTIVA_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("proactiva run")}`,
        `Reconfigure later: ${pc.cyan("proactiva configure")}`,
        `Diagnose setup: ${pc.cyan("proactiva doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.PROACTIVA_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Proactiva setup is ready.");
    return;
  }
  ...
  let llm: ProactivaConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@proactiva/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `proactiva doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("proactiva run")}`,
        `Then: ${pc.cyan("proactiva auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
