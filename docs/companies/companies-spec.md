...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Proactiva.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.proactiva.yaml`
...
- Proactiva must not require extra top-level fields for skill validity
- Proactiva-specific extensions must live under `metadata.proactiva` or `metadata.sources`
...
For Proactiva, the preferred fidelity extension is:
...
- issue/task Proactiva-only metadata
...
- Proactiva tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Proactiva shape:
...
Additional rules for Proactiva exporters:
...
- omit empty and default-valued Proactiva fields when possible
...
- Proactiva exporters should emit `.proactiva.yaml` as a sidecar by default
...
## 19. Proactiva Mapping

Proactiva can map this spec to its runtime model like this:
...
- Proactiva extension:
  - `.proactiva.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Proactiva-specific fidelity

Inline Proactiva-only metadata that must live inside a shared markdown file should use:

- `metadata.proactiva`

That keeps the base format broader than Proactiva.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Proactiva.

## 20. Cutover

Proactiva should cut over to this markdown-first package model as the primary portability format.

`proactiva.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Proactiva, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
