...
Build a custom adapter to connect Proactiva to any agent runtime.
...
|---|---|---|
| Source | Inside `proactiva-fork` | Separate npm package |
| Distribution | Ships with Proactiva | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Proactiva's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@proactiva/adapter-utils/server-utils`
2. Build environment with `buildProactivaEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@proactiva/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildProactivaEnv(agent)` | `@proactiva/adapter-utils/server-utils` | Inject `PROACTIVA_*` env vars |
| `renderTemplate(tpl, data)` | `@proactiva/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@proactiva/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@proactiva/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Proactiva's source:
...
`format-event.ts` — pretty-prints stdout for `proactiva run --watch` using `picocolors`.
...
With these flags set, the Proactiva UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Proactiva source changes required.
...
Make Proactiva skills discoverable to your agent runtime without writing to the agent's working directory:
