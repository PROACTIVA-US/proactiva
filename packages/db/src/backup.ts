...
function resolveProactivaInstanceId(): string {
  const raw = process.env.PROACTIVA_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveProactivaHomeDir(), "instances", resolveProactivaInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveProactivaHomeDir(), "instances", resolveProactivaInstanceId(), "data", "backups");
}
