...
Example Proactiva plugin that demonstrates:
...
```bash
pnpm --filter @proactiva/plugin-file-browser-example build
pnpm proactiva plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm proactiva plugin uninstall proactiva-file-browser-example --force
```
...
- Optional: use `proactiva-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
