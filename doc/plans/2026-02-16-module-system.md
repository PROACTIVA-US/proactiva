...
Proactiva's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Proactiva's API, UI, and data model | Yes |
...
modules/
  observability/
    proactiva.module.json     # manifest (required)
    src/
...
### Manifest (`proactiva.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@proactiva/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@proactiva/core";
...
```
1. Scan modules/ directory for proactiva.module.json manifests
...
```jsonc
// proactiva.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@proactiva/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@proactiva/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:proactiva/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.proactiva.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm proactiva store list                    # browse available modules and templates
pnpm proactiva store install <module-id>     # install a module
pnpm proactiva store import <template-id>    # import a company template
pnpm proactiva store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Proactiva instance | `server:started` |
...
4. **Module config** — `proactiva.config.json` with per-module config, env var interpolation
...
10. **`@proactiva/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
