...
Proactiva companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .proactiva.yaml     # Adapter config, env inputs, routines
```
...
- **.proactiva.yaml** holds Proactiva-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
proactiva company export <company-id> --out ./my-export
```
...
proactiva company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
proactiva company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
proactiva company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.proactiva.yaml`
...
```sh
# From a local folder
proactiva company import ./my-export

# From a GitHub URL
proactiva company import https://github.com/org/repo

# From a GitHub subfolder
proactiva company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
proactiva company import org/repo
proactiva company import org/repo/companies/acme
```
...
If `--target` is not specified, Proactiva infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
proactiva company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
proactiva company import org/company-templates/engineering-team \
  --target new \
...
```sh
proactiva company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
proactiva company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
proactiva company import ./package \
  --target new \
  --yes \
  --json
```
...
Proactiva supports several GitHub URL formats:
