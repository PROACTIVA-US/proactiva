...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@proactiva/db";
...
import { trackAgentCreated } from "@proactiva/shared/telemetry";
...
import { runClaudeLogin } from "@proactiva/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@proactiva/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@proactiva/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@proactiva/adapter-gemini-local";
...
export function agentRoutes(db: Db) {
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeProactivaSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readProactivaSkillSyncPreference(
