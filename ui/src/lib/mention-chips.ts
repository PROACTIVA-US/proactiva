...
import { parseAgentMentionHref, parseProjectMentionHref, parseSkillMentionHref, parseUserMentionHref } from "@proactiva/shared";
...
export function mentionChipInlineStyle(mention: ParsedMentionChip): CSSProperties | undefined {
  ...
  if (mention.kind === "project" && mention.color) {
    ...
    style["--proactiva-mention-project-color"] = mention.color;
  }

  if (mention.kind === "agent") {
    const iconMask = buildAgentIconMask(mention.icon);
    if (iconMask) {
      style["--proactiva-mention-icon-mask"] = iconMask;
    }
  }
...
export function applyMentionChipDecoration(element: HTMLElement, mention: ParsedMentionChip) {
  ...
  element.classList.add("proactiva-mention-chip", `proactiva-mention-chip--${mention.kind}`);
  if (mention.kind === "project") {
    element.classList.add("proactiva-project-mention-chip");
  }
...
export function clearMentionChipDecoration(element: HTMLElement) {
  ...
  element.classList.remove(
    "proactiva-mention-chip",
    "proactiva-mention-chip--agent",
    "proactiva-mention-chip--project",
    "proactiva-mention-chip--user",
    "proactiva-mention-chip--skill",
    "proactiva-project-mention-chip",
  );
  element.removeAttribute("contenteditable");
  element.style.removeProperty("border-color");
  element.style.removeProperty("background-color");
  element.style.removeProperty("color");
  element.style.removeProperty("--proactiva-mention-project-color");
  element.style.removeProperty("--proactiva-mention-icon-mask");
}
