...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Proactiva.
...
- current Proactiva plugin and adapter surfaces in this repo
...
## What Proactiva already has

Proactiva already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Proactiva

These ideas from `pi-mono` fit Proactiva with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Proactiva should continue exposing run and transcript events to plugins, for example:
...
This matches Proactiva's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Proactiva already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Proactiva already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Proactiva, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Proactiva core:

### 1. Arbitrary mutation hooks on control-plane decisions

Proactiva should not let general plugins rewrite:
...
Proactiva should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Proactiva is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Proactiva's board-and-issues model.

## Recommended Proactiva direction
...
If the question is "what works for Proactiva too?":
