...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".proactiva-sdk");
...
/**
 * Generate a complete Proactiva plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Proactiva plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "proactiva-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["proactiva", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@proactiva/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@proactiva/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import type { ProactivaPluginManifestV1 } from "@proactiva/plugin-sdk";

const manifest: ProactivaPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@proactiva/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@proactiva/plugin-sdk\` and \`@proactiva/shared\` from a local Proactiva checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.proactiva-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-proactiva-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <proactiva-sdk-path>]");
    process.exit(1);
  }
