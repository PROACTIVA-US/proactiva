...
const PROACTIVA_SESSIONS_DIR = path.join(os.homedir(), ".pi", "proactivas");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[proactiva] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[proactiva] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[proactiva] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(PROACTIVA_SESSIONS_DIR, { recursive: true });
  return PROACTIVA_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(PROACTIVA_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Inject skills
  const piSkillEntries = await readProactivaRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveProactivaDesiredSkillNames(config, piSkillEntries);
  ...
  if (workspaceCwd) env.PROACTIVA_WORKSPACE_CWD = workspaceCwd;
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[proactiva] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[proactiva] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const renderedBootstrapPrompt =
    !canResumeSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderProactivaWakePrompt(context.proactivaWake, { resumedSession: canResumeSession });
  ...
  const buildArgs = (sessionFile: string): string[] => {
    ...
    // Add Proactiva skills directory so Pi can load the proactiva skill
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[proactiva] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
