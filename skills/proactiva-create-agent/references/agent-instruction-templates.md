...
These templates are intentionally separate from the main Proactiva heartbeat skill so the core wake procedure stays short.
...
2. Copy that template into the new agent's instruction bundle, usually `AGENTS.md`. For hire requests using local managed-bundle adapters, this usually means setting the adapted template as `adapterConfig.promptTemplate`; Proactiva materializes it into `AGENTS.md`.
...
5. Keep the Proactiva heartbeat requirement and task-comment requirement.
