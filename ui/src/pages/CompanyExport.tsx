...
/**
 * Filter .proactiva.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterProactivaYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://proactiva.ing) package.");
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .proactiva.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .proactiva.yaml
    const yamlPath = exportData.proactivaExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterProactivaYaml(exportData.files[yamlPath], checkedFiles);
    }
