...
const DEFAULT_CLIENT_VERSION = "proactiva";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "proactiva";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`proactiva:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`proactiva:issue:${input.issueId}`, input.agentId);
  }
...
function resolveProactivaApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/proactiva-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildProactivaEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const proactivaApiUrlOverride = resolveProactivaApiUrlOverride(ctx.config.proactivaApiUrl);
  const proactivaEnv: Record<string, string> = {
    ...buildProactivaEnv(ctx.agent),
    PROACTIVA_RUN_ID: ctx.runId,
  };

  if (proactivaApiUrlOverride) {
    proactivaEnv.PROACTIVA_API_URL = proactivaApiUrlOverride;
  }
  if (wakePayload.taskId) proactivaEnv.PROACTIVA_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) proactivaEnv.PROACTIVA_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) proactivaEnv.PROACTIVA_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) proactivaEnv.PROACTIVA_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) proactivaEnv.PROACTIVA_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    proactivaEnv.PROACTIVA_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  proactivaEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/proactiva-claimed-api-key.json";
  const orderedKeys = [
    "PROACTIVA_RUN_ID",
    "PROACTIVA_AGENT_ID",
    ...
    "PROACTIVA_API_URL",
    "PROACTIVA_TASK_ID",
    "PROACTIVA_WAKE_REASON",
    "PROACTIVA_WAKE_COMMENT_ID",
    "PROACTIVA_APPROVAL_ID",
    "PROACTIVA_APPROVAL_STATUS",
    "PROACTIVA_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = proactivaEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = proactivaEnv.PROACTIVA_API_URL ?? "<set PROACTIVA_API_URL>";

  const lines = [
    "Proactiva wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `PROACTIVA_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load PROACTIVA_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $PROACTIVA_API_KEY on every API call.",
    "- Use X-Proactiva-Run-Id: $PROACTIVA_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: PROACTIVA_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$PROACTIVA_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$PROACTIVA_COMPANY_ID/issues?assigneeAgentId=$PROACTIVA_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardProactivaPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  proactivaEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateProactiva = parseObject(payloadTemplate.proactiva);
  const workspace = asRecord(ctx.context.proactivaWorkspace);
  const workspaces = Array.isArray(ctx.context.proactivaWorkspaces)
    ? ctx.context.proactivaWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.proactivaRuntimeServiceIntents)
    ? ctx.context.proactivaRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardProactiva: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: proactivaEnv.PROACTIVA_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.proactivaWake);
  if (Object.keys(structuredWake).length > 0) {
    standardProactiva.wake = structuredWake;
  }

  if (workspace) {
    standardProactiva.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardProactiva.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardProactiva.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "proactiva-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const proactivaEnv = buildProactivaEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderProactivaWakePrompt(ctx.context.proactivaWake);
  const structuredWakeJson = stringifyProactivaWakePayload(ctx.context.proactivaWake);
  ...
  const proactivaPayload = buildStandardProactivaPayload(ctx, wakePayload, proactivaEnv, payloadTemplate);
  ...
  agentParams.proactiva = proactivaPayload;
