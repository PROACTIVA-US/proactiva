const FAVICON_BLOCK_START = "<!-- PROACTIVA_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- PROACTIVA_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- PROACTIVA_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- PROACTIVA_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.PROACTIVA_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.PROACTIVA_WORKTREE_NAME) ?? nonEmpty(env.PROACTIVA_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.PROACTIVA_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="proactiva-worktree-enabled" content="true" />',
    `<meta name="proactiva-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="proactiva-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="proactiva-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
