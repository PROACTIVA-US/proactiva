/**
 * Core types for the Proactiva plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@proactiva/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @proactiva/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Proactiva object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Proactiva UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Proactiva domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Proactiva domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Proactiva secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Proactiva secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@proactiva/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@proactiva/plugin-sdk";
 *
