...
This document describes how `@proactiva/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Proactiva logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Proactiva `runId`)
