...
Agents need API keys to authenticate with Proactiva. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Proactiva
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Proactiva generates a token and passes it directly to the agent
...
`PROACTIVA_API_URL`, `PROACTIVA_AGENT_ID`, `PROACTIVA_COMPANY_ID`. We need to
add a `PROACTIVA_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Proactiva CLI
...
```
proactiva auth login
# Opens browser -> user confirms -> token stored at ~/.proactiva/credentials
```
...
managed by the Proactiva server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Proactiva.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Proactiva SKILL.md (or a link to it)
   - What information Proactiva needs from the agent (e.g. webhook URL, adapter
...
5. Proactiva stores the pending registration.
...
7. On approval, Proactiva provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Proactiva issues an API key (or JWT) upon approval, delivered
...
1. Generate a Proactiva invite link for the company.
...
4. A Proactiva company member approves the new agent.
5. Proactiva begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `PROACTIVA_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `proactiva auth login` for developer-managed remote agents.                                      |
