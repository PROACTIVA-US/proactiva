...
import { companies, companySkills, createDb } from "@proactiva/db";
import {
...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("proactiva-company-skills-service-");
  ...
  it("lists skills without exposing markdown content", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-heavy-skill-"));
