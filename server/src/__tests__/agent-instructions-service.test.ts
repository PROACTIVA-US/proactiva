...
describe("agent instructions service", () => {
  const originalProactivaHome = process.env.PROACTIVA_HOME;
  const originalProactivaInstanceId = process.env.PROACTIVA_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalProactivaHome === undefined) delete process.env.PROACTIVA_HOME;
    else process.env.PROACTIVA_HOME = originalProactivaHome;
    if (originalProactivaInstanceId === undefined) delete process.env.PROACTIVA_INSTANCE_ID;
    else process.env.PROACTIVA_INSTANCE_ID = originalProactivaInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const proactivaHome = await makeTempDir("proactiva-agent-instructions-home-");
    const externalRoot = await makeTempDir("proactiva-agent-instructions-external-");
    cleanupDirs.add(proactivaHome);
    cleanupDirs.add(externalRoot);
    process.env.PROACTIVA_HOME = proactivaHome;
    process.env.PROACTIVA_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("proactiva-agent-instructions-new-external-");
    cleanupDirs.add(proactivaHome);
    cleanupDirs.add(externalRoot);
    process.env.PROACTIVA_HOME = proactivaHome;
    process.env.PROACTIVA_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("proactiva-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const proactivaHome = await makeTempDir("proactiva-agent-instructions-recover-");
    cleanupDirs.add(proactivaHome);
    process.env.PROACTIVA_HOME = proactivaHome;
    process.env.PROACTIVA_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const proactivaHome = await makeTempDir("proactiva-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("proactiva-agent-instructions-stale-root-");
    cleanupDirs.add(proactivaHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const proactivaHome = await makeTempDir("proactiva-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("proactiva-agent-instructions-heal-write-stale-");
    cleanupDirs.add(proactivaHome);
    cleanupDirs.add(staleRoot);
    process.env.PROACTIVA_HOME = proactivaHome;
    process.env.PROACTIVA_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const proactivaHome = await makeTempDir("proactiva-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("proactiva-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(proactivaHome);
    cleanupDirs.add(staleRoot);
    process.env.PROACTIVA_HOME = proactivaHome;
    process.env.PROACTIVA_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const proactivaHome = await makeTempDir("proactiva-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("proactiva-agent-instructions-partial-root-");
    cleanupDirs.add(proactivaHome);
    cleanupDirs.add(staleRoot);
    process.env.PROACTIVA_HOME = proactivaHome;
    process.env.PROACTIVA_INSTANCE_ID = "test-instance";
