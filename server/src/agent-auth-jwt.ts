...
function jwtConfig() {
  const secret = process.env.PROACTIVA_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.PROACTIVA_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.PROACTIVA_AGENT_JWT_ISSUER ?? "proactiva",
    audience: process.env.PROACTIVA_AGENT_JWT_AUDIENCE ?? "proactiva-api",
  };
}
