...
base_cwd="${PROACTIVA_WORKSPACE_BASE_CWD:?PROACTIVA_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${PROACTIVA_WORKSPACE_CWD:?PROACTIVA_WORKSPACE_CWD is required}"
proactiva_home="${PROACTIVA_HOME:-$HOME/.proactiva}"
proactiva_instance_id="${PROACTIVA_INSTANCE_ID:-default}"
proactiva_dir="$worktree_cwd/.proactiva"
worktree_config_path="$proactiva_dir/config.json"
worktree_env_path="$proactiva_dir/.env"
worktree_name="${PROACTIVA_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${PROACTIVA_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.proactiva/config.json" || -L "$base_cwd/.proactiva/config.json" ) ]]; then
  source_config_path="$base_cwd/.proactiva/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$proactiva_home/instances/$proactiva_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$proactiva_dir"
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm proactiva --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm proactiva worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v proactiva >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      proactiva worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  PROACTIVA_DIR="$proactiva_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  PROACTIVA_WORKTREES_DIR="${PROACTIVA_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const proactivaDir = process.env.PROACTIVA_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.PROACTIVA_WORKTREES_DIR) ?? "~/.proactiva-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(proactivaDir, "config.json");
  const envPath = path.resolve(proactivaDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "proactiva",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.PROACTIVA_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.PROACTIVA_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.PROACTIVA_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "PROACTIVA_HOME=" + JSON.stringify(worktreeHome),
    "PROACTIVA_INSTANCE_ID=" + JSON.stringify(instanceId),
    "PROACTIVA_CONFIG=" + JSON.stringify(configPath),
    "PROACTIVA_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "PROACTIVA_IN_WORKTREE=true",
    "PROACTIVA_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.PROACTIVA_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("PROACTIVA_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  echo "Reusing existing isolated Proactiva worktree config at $worktree_config_path" >&2
else
  if proactiva_command_available; then
    run_isolated_worktree_init
  else
    echo "proactiva CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".proactiva-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
