...
Use when:
- You want Proactiva to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Proactiva server.
...
- proactivaApiUrl (string, optional): absolute Proactiva base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/proactiva-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default proactiva)

Standard outbound payload additions:
- proactiva (object): standardized Proactiva context added to every gateway agent request
- proactiva.workspace (object, optional): resolved execution workspace for this run
- proactiva.workspaces (array, optional): additional workspace hints Proactiva exposed to the run
- proactiva.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
