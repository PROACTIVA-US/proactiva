...
export const DEFAULT_WORKTREE_HOME = "~/.proactiva-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".proactiva");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: ProactivaConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "proactiva",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    PROACTIVA_HOME: paths.homeDir,
    PROACTIVA_INSTANCE_ID: paths.instanceId,
    PROACTIVA_CONFIG: paths.configPath,
    PROACTIVA_CONTEXT: paths.contextPath,
    PROACTIVA_IN_WORKTREE: "true",
    ...(branding?.name ? { PROACTIVA_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { PROACTIVA_WORKTREE_COLOR: branding.color } : {}),
  };
}
