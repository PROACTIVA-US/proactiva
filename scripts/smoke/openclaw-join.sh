...
COMPANY_ID="${COMPANY_ID:-${PROACTIVA_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-proactiva-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-proactiva-openclaw-smoke}"
...
Provide one of:
  PROACTIVA_AUTH_HEADER=\"Bearer <board-token>\"
  PROACTIVA_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Proactiva OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.proactiva.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
