...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "proactiva", LOGNAME: "proactiva", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "proactiva" }),
      },
    );

    expect(tokens).toEqual(["proactiva", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "proactiva" }, {
        userInfo: () => ({ username: "proactiva" }),
      });

      expect(tokens).toEqual(["proactiva", "custom-token"]);
    } finally {
