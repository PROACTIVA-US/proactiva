...
import type { Goal, Issue } from "@proactiva/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the Proactiva APIs.",
...
const plugin: ProactivaPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
