...
import type { CompanyPortabilityFileEntry } from "@proactiva/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "proactiva",
        name: "proactiva",
        description: "Proactiva coordination skill",
        markdown: "---\nname: proactiva\ndescription: Proactiva coordination skill\n---\n\n# Proactiva\n",
        sourceType: "github",
        sourceLocator: "https://github.com/PROACTIVA-US/proactiva/tree/master/skills/proactiva",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/PROACTIVA-US/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "proactiva",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/PROACTIVA-US/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "proactiva",
  ...
  it("exports referenced skills as stubs by default with sanitized Proactiva extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Proactiva"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${proactivaKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/proactiva/proactiva/proactiva/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/proactiva/proactiva/proactiva/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/proactiva/proactiva/proactiva/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".proactiva.yaml"]);
    expect(extension).toContain('schema: "proactiva/v1"');
    ...
    expect(extension).not.toContain("proactivaSkillSync");
  ...
  it("exports default sidebar order into the Proactiva extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".proactiva.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/proactiva/proactiva/proactiva/SKILL.md"])).toContain("# Proactiva");
    expect(asTextFile(exported.files["skills/proactiva/proactiva/proactiva/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/proactiva/proactiva/proactiva/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/proactiva/proactiva/proactiva/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/proactiva/proactiva/proactiva/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .proactiva.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".proactiva.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-proactiva",
        companyId: "company-1",
        key: "proactiva/proactiva/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/PROACTIVA-US/proactiva/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "proactiva_bundled",
          owner: "proactiva",
          repo: "proactiva",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/proactiva/proactiva/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/proactiva/proactiva/release-changelog/SKILL.md"])).toContain("proactiva/proactiva/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/proactiva",
            repoUrl: "https://github.com/PROACTIVA-US/proactiva.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .proactiva-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/PROACTIVA-US/proactiva.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/proactiva");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Proactiva",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/PROACTIVA-US/proactiva.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".proactiva.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/PROACTIVA-US/proactiva.git"');
    expect(extension).toContain('projectWorkspaceKey: "proactiva"');
  ...
  it("reads env inputs back from .proactiva.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .proactiva.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Proactiva routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "proactiva-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Proactiva" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Proactiva"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .proactiva.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Proactiva");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".proactiva.yaml"] = `${exported.files[".proactiva.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
