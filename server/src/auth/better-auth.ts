...
import { resolveProactivaInstanceId } from "../home-paths.js";
...
export function deriveAuthCookiePrefix(instanceId = resolveProactivaInstanceId()): string {
  const scopedInstanceId = instanceId
    .trim()
    .replace(AUTH_COOKIE_PREFIX_INVALID_SEGMENTS_RE, "-")
    .replace(/^-+|-+$/g, "") || AUTH_COOKIE_PREFIX_FALLBACK;
  return `proactiva-${scopedInstanceId}`;
}
...
export function createBetterAuthInstance(db: Db, config: Config, trustedOrigins?: string[]): BetterAuthInstance {
  const baseUrl = config.authBaseUrlMode === "explicit" ? config.authPublicBaseUrl : undefined;
  const secret = process.env.BETTER_AUTH_SECRET ?? process.env.PROACTIVA_AGENT_JWT_SECRET;
  if (!secret) {
    throw new Error(
      "BETTER_AUTH_SECRET (or PROACTIVA_AGENT_JWT_SECRET) must be set. " +
      "For local development, set BETTER_AUTH_SECRET=proactiva-dev-secret in your .env file.",
    );
  }
  const effectiveTrustedOrigins = trustedOrigins ?? deriveAuthTrustedOrigins(config);

  const publicUrl = process.env.PROACTIVA_PUBLIC_URL ?? baseUrl;
