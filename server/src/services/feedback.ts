...
import { readProactivaSkillSyncPreference } from "@proactiva/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@proactiva/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@proactiva/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@proactiva/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveProactivaInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "proactiva-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "proactiva-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "proactiva-feedback-v1";
const FEEDBACK_DESTINATION = "proactiva_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "proactiva_run" &&
    file.source !== "proactiva_run_events" &&
    file.source !== "proactiva_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.PROACTIVA_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readProactivaSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let proactivaRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      proactivaRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "proactiva/run.json",
        contentType: "application/json",
        source: "proactiva_run",
        contents: `${JSON.stringify(proactivaRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.proactivaRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "proactiva/run-events.json",
        contentType: "application/json",
        source: "proactiva_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "proactiva/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "proactiva_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.proactivaRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
