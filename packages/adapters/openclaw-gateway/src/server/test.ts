...
import { asString, parseObject } from "@proactiva/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Proactiva server host.",
        });
      }
    } catch (err) {
      checks.push({
