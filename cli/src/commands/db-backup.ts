...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@proactiva/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://proactiva:proactiva@127.0.0.1:${port}/proactiva`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" proactiva db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveProactivaInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "proactiva";
