...
Proactiva supports two runtime modes:
...
Proactiva now treats **bind** as a separate concern from auth:
...
pnpm proactiva onboard --yes --bind lan
pnpm proactiva run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Proactiva emits a startup warning with a one-time high-entropy claim URL.
