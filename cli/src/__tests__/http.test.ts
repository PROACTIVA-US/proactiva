...
import { ApiConnectionError, ApiRequestError, ProactivaApiClient } from "../client/http.js";

describe("ProactivaApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new ProactivaApiClient({
    ...
    expect(headers["x-proactiva-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new ProactivaApiClient({ apiBase: "http://localhost:3100" });
