...
This document defines the V1 design for a Proactiva adapter that integrates with
...
4. synthesized stdout events for Proactiva UI/CLI
...
3. **Agent callback auth to Proactiva** uses a bootstrap exchange flow (no long-lived Proactiva key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Proactiva endpoints, not full SKILL.md prompt injection.
...
- `proactivaPublicUrl` (optional override; else `PROACTIVA_PUBLIC_URL` env)
...
## Proactiva Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `PROACTIVA_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.proactivaPublicUrl`
2. `process.env.PROACTIVA_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Proactiva credentials in prompt text.

Flow:

1. Before launch/follow-up, Proactiva mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Proactiva validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Proactiva API calls.
...
1. Prompt includes a compact instruction to fetch skills from Proactiva.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/proactiva`
   - `GET /api/skills/proactiva-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <proactivaPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Proactiva run context.
...
5. `proactivaPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Proactiva skill endpoints |
| Proactiva auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Proactiva context.
3. Add richer PR metadata surfacing in Proactiva UI.
