...
Today release smoke testing for published Proactiva packages is manual and shell-driven:
...
- Docker
- `npx proactiva@canary`
- `npx proactiva@latest`
...
- runs `proactiva@${PROACTIVAAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@proactiva.local`
- password: `proactiva-smoke-password`
...
- the suite passes locally against both:
  - `PROACTIVAAI_VERSION=canary`
  - `PROACTIVAAI_VERSION=latest`
...
1. A published `proactiva@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `proactiva@latest` Docker install can be smoke-tested by Playwright in CI.
