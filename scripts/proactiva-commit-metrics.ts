...
const DEFAULT_QUERY = "\"Co-Authored-By: Proactiva <noreply@proactiva.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/proactiva-commit-metrics-cache.json");
...
const PROACTIVA_EMAIL = "noreply@proactiva.ing";
const PROACTIVA_NAME = "proactiva";
...
function printHelp() {
  console.log(`Usage: tsx scripts/proactiva-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === PROACTIVA_EMAIL) || (displayName && displayName.toLowerCase() === PROACTIVA_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Proactiva commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "proactiva-commit-metrics",
