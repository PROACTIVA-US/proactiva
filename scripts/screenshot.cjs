#!/usr/bin/env node
/**
 * Screenshot utility for Proactiva UI.
 *
 * Reads the board token from ~/.proactiva/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/proactiva-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".proactiva/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.proactiva/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Proactiva origin only
