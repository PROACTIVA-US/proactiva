...
- New config/env settings:
  - `PROACTIVA_AGENT_JWT_SECRET`
  - `PROACTIVA_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `PROACTIVA_AGENT_JWT_ISSUER` (default: `proactiva`)
  - `PROACTIVA_AGENT_JWT_AUDIENCE` (default: `proactiva-api`)
...
1. In:
   ...
   inject `PROACTIVA_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `PROACTIVA_API_KEY`, do not overwrite it.
- Continue injecting:
  - `PROACTIVA_AGENT_ID`
...
- Local adapters authenticate without manual `PROACTIVA_API_KEY` config.
