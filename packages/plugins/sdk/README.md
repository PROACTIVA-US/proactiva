# `@proactiva/plugin-sdk`

Official TypeScript SDK for Proactiva plugin authors.

- **Worker SDK:** `@proactiva/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@proactiva/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@proactiva/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@proactiva/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@proactiva/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@proactiva/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@proactiva/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@proactiva/plugin-sdk/ui/hooks` | Hooks only |
| `@proactiva/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@proactiva/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@proactiva/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@proactiva/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@proactiva/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@proactiva/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Proactiva app. They can call ordinary Proactiva HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Proactiva instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@proactiva/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@proactiva/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Proactiva information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@proactiva/plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:proactiva.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@proactiva/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@proactiva/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@proactiva/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@proactiva/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@proactiva/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@proactiva/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@proactiva/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@proactiva/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@proactiva/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@proactiva/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@proactiva/plugin-sdk/bundlers";
...
```bash
proactiva-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@proactiva/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__proactiva__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__proactiva__/events` streams `reload` SSE events on UI build changes
