 ...
 */
import { pluginManifestV1Schema } from "@proactiva/shared";
import type { ProactivaPluginManifestV1 } from "@proactiva/shared";
import { PLUGIN_API_VERSION } from "@proactiva/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): ProactivaPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as ProactivaPluginManifestV1,
        };
      }
