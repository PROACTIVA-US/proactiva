...
## Scope
This plan is now **gateway-only**. Proactiva supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Proactiva skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Proactiva task.
...
```bash
pnpm --filter @proactiva/server test:run -- openclaw-gateway
pnpm --filter @proactiva/server typecheck
pnpm --filter @proactiva/ui typecheck
pnpm --filter proactiva typecheck
```
