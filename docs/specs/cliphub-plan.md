# ClipHub: Marketplace for Proactiva Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Proactiva configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Proactiva-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Proactiva company (agents, projects, governance) |
...
A complete Proactiva company configuration:
...
Single-agent configurations designed to plug into a Proactiva org:
...
Portable skill files that any Proactiva agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Proactiva's skill loading system
...
interface Listing {
  ...
  proactivaVersionMin: string;     // Minimum Proactiva version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Proactiva company
...
| `POST` | `/api/listings/:id/install` | Install to Proactiva company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Proactiva company (or create new)
  → ClipHub API calls Proactiva API to:
  ...
  → Redirect to Proactiva dashboard with new team running
```
...
### 5.3 Creator: Export from Proactiva → Publish

```
Running Proactiva company → "Export as Blueprint" (CLI or UI)
  → Proactiva exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Proactiva brand blue/purple)
...
| Install flow | One-click deploy to Proactiva company |
...
{
  "targetCompanyId": "uuid",         // Existing Proactiva company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `proactiva-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Proactiva
- **Backend**: Node.js API (or extend Proactiva server)
- **Database**: Postgres (can share Proactiva's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Proactiva auth (or OAuth2)

### 9.2 Integration with Proactiva

ClipHub can be:
- **Option A**: A separate app that calls Proactiva's API to install blueprints
- **Option B**: A built-in section of the Proactiva UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Proactiva UI and API.
...
- [ ] Install flow: blueprint → Proactiva company
...
- [ ] "Export from Proactiva" CLI command
