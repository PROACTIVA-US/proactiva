...
This document defines the rollout plan for adapter-wide skill support in Proactiva.
...
- Proactiva stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Proactiva currently has these adapters:
...
1. Desired skills live in Proactiva for every adapter.
...
These adapters have a stable local skills directory that Proactiva can read and manage.
...
These adapters do not have a meaningful Proactiva-owned persistent install state.
...
- desired skills stored in Proactiva
...
- runtime already injects Proactiva skills into `~/.cursor/skills`
...
- runtime already injects Proactiva skills into `~/.gemini/skills`
...
- runtime already injects Proactiva skills into `~/.pi/agent/skills`
...
- Proactiva can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Proactiva skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Proactiva may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Proactiva skills that are clearly marked as Proactiva-managed
...
- Proactiva stores desired skills only
...
- avoid destructive wording unless Proactiva can prove a skill is Proactiva-managed
...
That gets Proactiva from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
