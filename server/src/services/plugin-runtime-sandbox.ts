...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __proactiva_exports: module.exports,
      __proactiva_module: module,
      __proactiva_require: requireInSandbox,
      __proactiva_filename: realPath,
      __proactiva_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__proactiva_exports, __proactiva_module, __proactiva_require, __proactiva_filename, __proactiva_dirname)`;
