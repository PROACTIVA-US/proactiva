...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[proactiva] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`proactiva worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const ignoredRelativePaths = new Set([
  ".proactiva/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[proactiva] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[proactiva] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[proactiva] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  PROACTIVA_UI_DEV_MIDDLEWARE: "true",
};
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[proactiva] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.PROACTIVA_BIND = effectiveBind;
  if (bindHost) {
    env.PROACTIVA_BIND_HOST = bindHost;
  } else {
    delete env.PROACTIVA_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.PROACTIVA_DEPLOYMENT_MODE;
    delete env.PROACTIVA_DEPLOYMENT_EXPOSURE;
    delete env.PROACTIVA_AUTH_BASE_URL_MODE;
    console.log("[proactiva] dev mode: local_trusted (bind=loopback)");
  } else {
    env.PROACTIVA_DEPLOYMENT_MODE = "authenticated";
    env.PROACTIVA_DEPLOYMENT_EXPOSURE = "private";
    env.PROACTIVA_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[proactiva] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.PROACTIVA_BIND;
  delete env.PROACTIVA_BIND_HOST;
  delete env.PROACTIVA_DEPLOYMENT_MODE;
  delete env.PROACTIVA_DEPLOYMENT_EXPOSURE;
  delete env.PROACTIVA_AUTH_BASE_URL_MODE;
  console.log("[proactiva] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[proactiva] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@proactiva/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[proactiva] Command failed with code ${status.code}: pnpm --filter @proactiva/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[proactiva] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.PROACTIVA_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[proactiva] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[proactiva] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@proactiva/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[proactiva] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@proactiva/server", serverScript, ...forwardedArgs],
