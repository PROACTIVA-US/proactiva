...
import { companySkills } from "@proactiva/db";
import { readProactivaSkillSyncPreference } from "@proactiva/adapter-utils/server-utils";
import type { ProactivaSkillEntry } from "@proactiva/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@proactiva/shared";
import { findActiveServerAdapter } from "../adapters/index.js";
import { resolveProactivaInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("proactiva/proactiva/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveProactivaInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "proactiva_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Proactiva skills are read-only.",
      sourceLabel: "Proactiva bundled",
      sourceBadge: "proactiva",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Proactiva workspace",
        sourceBadge: "proactiva",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "proactiva_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Proactiva skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "proactiva_bundled"
        && incomingKind === "github"
        && incomingOwner === "proactiva"
        && incomingRepo === "proactiva"
      ) {
        out.push(existing);
        continue;
      }
