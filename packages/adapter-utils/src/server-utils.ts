...
const PROACTIVA_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export const DEFAULT_PROACTIVA_AGENT_PROMPT_TEMPLATE = [
...
export interface ProactivaSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: ProactivaSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "proactiva_required",
      originLabel: "Required by Proactiva",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Proactiva",
    readOnly: false,
  };
}
...
type ProactivaWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type ProactivaWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: ProactivaWakeExecutionPrincipal | null;
  returnAssignee: ProactivaWakeExecutionPrincipal | null;
  lastDecisionOutcome: string | null;
  allowedActions: string[];
};

type ProactivaWakeComment = {
  id: string | null;
  issueId: string | null;
  body: string;
...
type ProactivaWakeContinuationSummary = {
  key: string | null;
  title: string | null;
  body: string;
  bodyTruncated: boolean;
...
type ProactivaWakeLivenessContinuation = {
  attempt: number | null;
...
type ProactivaWakeChildIssueSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type ProactivaWakeBlockerSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type ProactivaWakePayload = {
  reason: string | null;
  issue: ProactivaWakeIssue | null;
  checkedOutByHarness: boolean;
  dependencyBlockedInteraction: boolean;
  unresolvedBlockerIssueIds: string[];
  unresolvedBlockerSummaries: ProactivaWakeBlockerSummary[];
  executionStage: ProactivaWakeExecutionStage | null;
  continuationSummary: ProactivaWakeContinuationSummary | null;
  livenessContinuation: ProactivaWakeLivenessContinuation | null;
  childIssueSummaries: ProactivaWakeChildIssueSummary[];
  childIssueSummaryTruncated: boolean;
  commentIds: string[];
  latestCommentId: string | null;
  comments: ProactivaWakeComment[];
...
function normalizeProactivaWakeIssue(value: unknown): ProactivaWakeIssue | null {
...
function normalizeProactivaWakeComment(value: unknown): ProactivaWakeComment | null {
...
function normalizeProactivaWakeContinuationSummary(value: unknown): ProactivaWakeContinuationSummary | null {
...
function normalizeProactivaWakeLivenessContinuation(value: unknown): ProactivaWakeLivenessContinuation | null {
...
function normalizeProactivaWakeChildIssueSummary(value: unknown): ProactivaWakeChildIssueSummary | null {
...
function normalizeProactivaWakeBlockerSummary(value: unknown): ProactivaWakeBlockerSummary | null {
...
function normalizeProactivaWakeExecutionPrincipal(value: unknown): ProactivaWakeExecutionPrincipal | null {
...
function normalizeProactivaWakeExecutionStage(value: unknown): ProactivaWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeProactivaWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizeProactivaWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizeProactivaWakePayload(value: unknown): ProactivaWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeProactivaWakeComment(entry))
        .filter((entry): entry is ProactivaWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeProactivaWakeExecutionStage(payload.executionStage);
  const continuationSummary = normalizeProactivaWakeContinuationSummary(payload.continuationSummary);
  const livenessContinuation = normalizeProactivaWakeLivenessContinuation(payload.livenessContinuation);
  const childIssueSummaries = Array.isArray(payload.childIssueSummaries)
    ? payload.childIssueSummaries
        .map((entry) => normalizeProactivaWakeChildIssueSummary(entry))
        .filter((entry): entry is ProactivaWakeChildIssueSummary => Boolean(entry))
    : [];
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ? payload.unresolvedBlockerSummaries
        .map((entry) => normalizeProactivaWakeBlockerSummary(entry))
        .filter((entry): entry is ProactivaWakeBlockerSummary => Boolean(entry))
    : [];

  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !executionStage && !continuationSummary && !livenessContinuation && !normalizeProactivaWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizeProactivaWakeIssue(payload.issue),
...
export function stringifyProactivaWakePayload(value: unknown): string | null {
  const normalized = normalizeProactivaWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function renderProactivaWakePrompt(
  value: unknown,
  ...
  const principalLabel = (principal: ProactivaWakeExecutionPrincipal | null) => {
  ...
  const lines = resumedSession
      ? [
        "## Proactiva Resume Delta",
        "",
        "You are resuming an existing Proactiva session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "PROACTIVA_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildProactivaEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    PROACTIVA_AGENT_ID: agent.id,
    PROACTIVA_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.PROACTIVA_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.PROACTIVA_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.PROACTIVA_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.PROACTIVA_API_URL = apiUrl;
  return vars;
}
...
export async function resolveProactivaSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...PROACTIVA_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listProactivaSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<ProactivaSkillEntry[]> {
  const root = await resolveProactivaSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `proactiva/proactiva/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Proactiva skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Proactiva cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredProactivaRuntimeSkills(value: unknown): ProactivaSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: ProactivaSkillEntry[] = [];
...
export async function readProactivaRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<ProactivaSkillEntry[]> {
  const configuredEntries = normalizeConfiguredProactivaRuntimeSkills(config.proactivaRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listProactivaSkillEntries(moduleDir, additionalCandidates);
}

export async function readProactivaSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listProactivaSkillEntries(moduleDir);
...
export function readProactivaSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.proactivaSkillSync;
...
function canonicalizeDesiredProactivaSkillReference(
  reference: string,
...
export function resolveProactivaDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readProactivaSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredProactivaSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeProactivaSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.proactivaSkillSync;
  const current =
  ...
  next.proactivaSkillSync = current;
  return next;
}

export async function ensureProactivaSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Proactiva server itself is started from
    // within a Claude Code session (e.g. `npx proactiva run` in a terminal
