...
import { buildProjectMentionHref, buildSkillMentionHref } from "@proactiva/shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Proactiva App", "Ping @Proactiva App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Proactiva App",
      atPos: 5,
      endPos: "Ping @Proactiva App".length,
    });
  });
  ...
  it("accepts mention selection from touchstart taps", async () => {
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("Proactiva App"));
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Proactiva App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
