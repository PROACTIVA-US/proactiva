import { asBoolean, asString, asStringArray } from "@proactiva/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Proactiva will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
