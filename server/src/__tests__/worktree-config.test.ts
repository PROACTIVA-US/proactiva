...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const proactivaDir = path.join(worktreeRoot, ".proactiva");
    const configPath = path.join(proactivaDir, "config.json");
    const envPath = path.join(proactivaDir, ".env");
    const sharedRoot = path.join(tempRoot, ".proactiva", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".proactiva-worktrees");

    await fs.mkdir(proactivaDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Proactiva environment variables",
        "PROACTIVA_IN_WORKTREE=true",
        "PROACTIVA_WORKTREE_NAME=PAP-884-ai-commits-component",
        "PROACTIVA_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.PROACTIVA_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.PROACTIVA_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`PROACTIVA_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('PROACTIVA_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`PROACTIVA_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`PROACTIVA_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('PROACTIVA_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.PROACTIVA_HOME).toBe(isolatedHome);
    expect(process.env.PROACTIVA_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Proactiva environment variables",
        "PROACTIVA_IN_WORKTREE=true",
        "PROACTIVA_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.PROACTIVA_IN_WORKTREE = "true";
    process.env.PROACTIVA_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.PROACTIVA_WORKTREES_DIR = isolatedHome;
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-worktree-runtime-override-"));
    const isolatedHome = path.join(tempRoot, ".proactiva-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        "# Proactiva environment variables",
        `PROACTIVA_HOME=${JSON.stringify(isolatedHome)}`,
        `PROACTIVA_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `PROACTIVA_CONFIG=${JSON.stringify(configPath)}`,
        `PROACTIVA_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'PROACTIVA_IN_WORKTREE="true"',
        'PROACTIVA_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.PROACTIVA_IN_WORKTREE = "true";
    process.env.PROACTIVA_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.PROACTIVA_HOME = transientHome;
    ...
    expect(repairedEnv).toContain(`PROACTIVA_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`PROACTIVA_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.PROACTIVA_HOME).toBe(isolatedHome);
  });

  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".proactiva-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".proactiva", "worktrees");
    ...
    const proactivaDir = path.join(currentWorktreeRoot, ".proactiva");
    const configPath = path.join(proactivaDir, "config.json");
    const envPath = path.join(proactivaDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".proactiva", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "proactiva-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.PROACTIVA_IN_WORKTREE = "true";
    process.env.PROACTIVA_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.PROACTIVA_HOME = isolatedHome;
    process.env.PROACTIVA_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.PROACTIVA_CONFIG = configPath;
