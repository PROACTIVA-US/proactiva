...
This document defines the product and UI plan for skill management in Proactiva.
...
- Proactiva company import/export can include skills as package content
...
3. The UI must always tell the truth about what Proactiva knows:
   - desired state in Proactiva
...
Proactiva should treat skills at two scopes:
...
- created locally inside Proactiva later
...
7. “Understand what Proactiva knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Proactiva-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Proactiva did not install.

These should be clearly marked:

- external
- not managed by Proactiva
...
- `desired_only`: Proactiva wants it, adapter does not show it yet
- `external`: adapter has it but Proactiva does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Proactiva only tracks desired state
...
- Proactiva can still track desired skills
...
- Proactiva can see adapter skills
...
- `.proactiva.yaml` may add Proactiva-specific fidelity, but should not replace the base shortname association model
...
- `.proactiva.yaml` may add Proactiva-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Proactiva should have a company-scoped skill model or managed package model representing:
...
Proactiva should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Proactiva-specific.
...
That gives Proactiva one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
