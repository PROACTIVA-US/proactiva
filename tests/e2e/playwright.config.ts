...
const PORT = Number(process.env.PROACTIVA_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const PROACTIVA_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp PROACTIVA_HOME.
  webServer: {
    command: `pnpm proactiva onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active Proactiva home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      PROACTIVA_HOME,
      PROACTIVA_INSTANCE_ID: "playwright-e2e",
      PROACTIVA_BIND: "loopback",
      PROACTIVA_DEPLOYMENT_MODE: "local_trusted",
