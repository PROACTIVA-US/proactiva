...
Proactiva exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`PROACTIVA_API_KEY`)
...
- Run audit trail: include `X-Proactiva-Run-Id` header on all mutating requests during heartbeats
