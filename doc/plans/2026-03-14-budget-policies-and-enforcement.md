...
Proactiva already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Proactiva should implement next.

## Product Goals

Proactiva should let operators:
...
For the next implementation pass, Proactiva should enforce these defaults:
...
Proactiva should separate subscription-included usage from billed spend:
...
Proactiva should have two threshold classes:
...
Proactiva must know that a project is budget-paused and enforce it.
...
Proactiva must also block execution before new work starts.
