...
This spec defines how Proactiva actually runs agents while staying runtime-agnostic.
...
1. Proactiva is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Proactiva manages lifecycle, and on exit Proactiva parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `proactiva.skill` (shared Proactiva skill text block)
...
2. Preferred transport is env vars (`PROACTIVA_*`) injected at runtime.
