...
Add a single storage subsystem for Proactiva that supports:
...
- API serves attachment bytes through authenticated Proactiva endpoints.
...
- Default local path is under instance root: `~/.proactiva/instances/<instanceId>/data/storage`.
...
- `proactiva onboard` writes a valid `storage` config block by default.
- `proactiva configure --section storage` can switch between local and s3 modes.
...
- `proactiva doctor` reports actionable storage status.
