...
`PAP-447` asks how Proactiva should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Proactiva instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Proactiva:

- some operators use Proactiva to manage Proactiva and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Proactiva
- Claude and Codex expose different built-in affordances, so Proactiva should not overfit to one tool

## Core Product Decision

Proactiva should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Proactiva
...
- Proactiva should not assume that every adapter can see or use a host filesystem path directly
...
If Proactiva makes worktrees agent-first, it will blur:
...
- Proactiva core resolves and records execution workspace state
...
That gives Proactiva room to support:
...
- Proactiva handles workspace runtime services under the hood using project/workspace policy
...
Once Proactiva is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Proactiva should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Proactiva cannot infer
...
- when issue moves to done, should Proactiva auto-commit?
- should it auto-push?
...
Proactiva already has the right foundation for a project-first model.
...
For server-spawned local adapters, Proactiva already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Proactiva should mirror directly

Implication:

- for `codex_local`, Proactiva should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Proactiva should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Proactiva should separate:
...
Proactiva should be able to express intentions such as:
...
These are not specific to Proactiva. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".proactiva/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Proactiva-specific `devServer` flag.
...
Proactiva should distinguish between:
...
Proactiva should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Proactiva should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Proactiva
...
For runtime services specifically, Proactiva should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Proactiva-only dev-server flags
...
3. Inject resolved execution workspace details into `context.proactivaWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Proactiva instance can be reused through env/config injection when appropriate
...
- Proactiva can decide whether to reuse or start a fresh service deterministically
...
3. Test against Proactiva-on-Proactiva workflows first.
...
7. Workspace runtime services are modeled generically, not as Proactiva-specific dev-server toggles.
