...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Proactiva OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/proactiva");
    expect(text).toContain("Suggested Proactiva base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("proactivaApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.proactivaApiUrl");
    expect(text).toContain("~/.openclaw/workspace/proactiva-claimed-api-key.json");
    expect(text).toContain("PROACTIVA_API_KEY");
