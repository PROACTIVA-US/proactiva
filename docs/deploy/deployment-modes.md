...
Proactiva supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm proactiva allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Proactiva emits a one-time claim URL at startup:
...
```sh
PROACTIVA_DEPLOYMENT_MODE=authenticated PROACTIVA_BIND=lan pnpm proactiva run
```
