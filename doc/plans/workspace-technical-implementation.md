...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/proactiva-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `PROACTIVA_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `PROACTIVA_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Proactiva may create host-local worktrees, processes, and paths
- `cloud`
  - Proactiva should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `proactiva | github | vercel | s3 | custom`
...
- use `PROACTIVA_EXECUTION_TOPOLOGY` as a defaulting guardrail
