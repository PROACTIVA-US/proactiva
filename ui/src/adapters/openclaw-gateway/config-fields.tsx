...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Proactiva API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "proactivaApiUrl",
                  String(config.proactivaApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "proactivaApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://proactiva.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/proactiva-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "proactiva"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Proactiva persists a device key during onboarding so pairing approvals
