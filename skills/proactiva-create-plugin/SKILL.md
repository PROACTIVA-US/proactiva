---
name: proactiva-create-plugin
description: >
  Create new Proactiva plugins with the current alpha SDK/runtime. Use when
...
# Create a Proactiva Plugin

Use this skill when the task is to create, scaffold, or document a Proactiva plugin.
...
node packages/plugins/create-proactiva-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Proactiva repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.proactiva-sdk/`:

```bash
pnpm --filter @proactiva/create-proactiva-plugin build
node packages/plugins/create-proactiva-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Proactiva from an absolute local path during development
