...
Use when:
- You want Proactiva to run Cursor Agent CLI locally as the agent runtime
...
- Proactiva auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$proactiva" and related skills on local runs.
- Proactiva auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
