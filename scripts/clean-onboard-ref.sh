...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Proactiva data dir to use
  PROACTIVA_HOST=127.0.0.1    Host passed to the onboarded server
  PROACTIVA_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/proactiva-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/proactiva-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export PROACTIVA_HOST="${PROACTIVA_HOST:-127.0.0.1}"
export PROACTIVA_PORT="${PROACTIVA_PORT:-3100}"
export PROACTIVA_OPEN_ON_LISTEN="${PROACTIVA_OPEN_ON_LISTEN:-false}"
...
echo "PROACTIVA_HOST: $PROACTIVA_HOST"
echo "PROACTIVA_PORT: $PROACTIVA_PORT"
...
env \
  ...
  HOST="$PROACTIVA_HOST" \
  PORT="$PROACTIVA_PORT" \
  PROACTIVA_OPEN_ON_LISTEN="$PROACTIVA_OPEN_ON_LISTEN" \
  pnpm proactiva onboard --yes --data-dir "$PC_DATA"
