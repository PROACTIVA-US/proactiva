...
export class ProactivaApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-proactiva-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Proactiva API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Proactiva server is not running, the configured URL is wrong, or the request is being blocked before it reaches Proactiva.",
    "",
    "Try:",
    "- Start Proactiva with `pnpm dev` or `pnpm proactiva run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Proactiva is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PROACTIVA_API_URL\`.`,
  );
  return lines.join("\n");
}
