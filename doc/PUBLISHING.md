...
Low-level reference for how Proactiva packages are prepared and published to npm.
...
Proactiva no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `proactiva`, imports code from workspace packages such as:
...
### Manual first publish for `@proactiva/ui`
...
npm view @proactiva/ui version

# make sure the dist payload is fresh
pnpm --filter @proactiva/ui build
...
- If `npm view @proactiva/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@proactiva/ui`, the `proactiva` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Proactiva uses calendar versions:
