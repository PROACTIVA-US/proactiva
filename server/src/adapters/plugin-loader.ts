...
function extractUiParserSource(
  ...
  const contractVersion = pkg.proactiva?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no proactiva.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
