...
For `claude_local` and `codex_local`, Proactiva assumes the CLI is already installed and authenticated on the host machine.
...
Proactiva stores resumable session state per `(agent, taskKey, adapterType)`.
