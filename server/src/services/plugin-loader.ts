 ...
 *    (`~/.proactiva/plugins/`) and `node_modules` for packages matching
 *    the `proactiva-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Proactiva plugins.
 * Packages matching this pattern are considered Proactiva plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "proactiva-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: ProactivaPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.proactiva/plugins/ local directory
  | "npm"               // npm packages matching proactiva-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: ProactivaPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the proactiva-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "proactiva-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: ProactivaPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Proactiva plugins installed as npm packages in the current
   * Node.js environment matching the "proactiva-plugin-*" naming convention.
   *
   ...
   * the "proactivaPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Proactiva plugin.
   * Throws if the package is a Proactiva plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<ProactivaPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: ProactivaPluginManifestV1;
    newManifest: ProactivaPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Proactiva plugin naming convention.
 * Accepts both the "proactiva-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @proactiva/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "proactivaPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const proactivaPlugin = pkgJson["proactivaPlugin"];
  if (
    proactivaPlugin !== null &&
    typeof proactivaPlugin === "object" &&
    !Array.isArray(proactivaPlugin)
  ) {
    const manifestRelPath = (proactivaPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: ProactivaPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumProactivaVersion;
}
 ...
 *   packageName: "proactiva-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: ProactivaPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as ProactivaPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Proactiva plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<ProactivaPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Proactiva plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasProactivaPlugin = "proactivaPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasProactivaPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<ProactivaPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: ProactivaPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @proactiva/shared exports). Run those workers through
