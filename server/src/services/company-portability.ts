...
import type { Db } from "@proactiva/db";
import type {
...
import {
  readProactivaSkillSyncPreference,
  writeProactivaSkillSyncPreference,
} from "@proactiva/adapter-utils/server-utils";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@proactiva/adapter-opencode-local/server";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".proactiva.yaml" || normalized === ".proactiva.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const proactiva = isPlainRecord(metadata?.proactiva) ? metadata?.proactiva as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.proactivaSkillKey)
    ?? asString(proactiva?.skillKey)
    ?? asString(proactiva?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "proactiva_bundled") {
    return `proactiva/proactiva/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "proactiva" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .proactiva.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .proactiva.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .proactiva.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .proactiva.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Proactiva will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .proactiva.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .proactiva.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .proactiva.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .proactiva.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .proactiva.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .proactiva.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .proactiva.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .proactiva.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .proactiva.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[proactivaExtensionPath];
  if (selectedFiles.has(proactivaExtensionPath) && typeof extensionEntry === "string") {
    filtered[proactivaExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findProactivaExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".proactiva.yaml"] === "string") return ".proactiva.yaml";
  if (typeof files[".proactiva.yml"] === "string") return ".proactiva.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.proactiva.yaml") || entry.endsWith("/.proactiva.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "proactiva_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "proactiva/proactiva",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/PROACTIVA-US/proactiva/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "proactiva_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.proactivaSkillKey = skill.key;
  metadata.proactiva = {
    ...(isPlainRecord(metadata.proactiva) ? metadata.proactiva : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const proactivaExtensionPath = findProactivaExtensionPath(normalizedFiles);
  const proactivaExtension = proactivaExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, proactivaExtensionPath) ?? "")
    : {};
  const proactivaCompany = isPlainRecord(proactivaExtension.company) ? proactivaExtension.company : {};
  const proactivaSidebar = normalizePortableSidebarOrder(proactivaExtension.sidebar);
  const proactivaAgents = isPlainRecord(proactivaExtension.agents) ? proactivaExtension.agents : {};
  const proactivaProjects = isPlainRecord(proactivaExtension.projects) ? proactivaExtension.projects : {};
  const proactivaTasks = isPlainRecord(proactivaExtension.tasks) ? proactivaExtension.tasks : {};
  const proactivaRoutines = isPlainRecord(proactivaExtension.routines) ? proactivaExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(proactivaCompany.brandColor),
      logoPath: asString(proactivaCompany.logoPath) ?? asString(proactivaCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof proactivaCompany.requireBoardApprovalForNewAgents === "boolean"
          ? proactivaCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof proactivaCompany.feedbackDataSharingEnabled === "boolean"
          ? proactivaCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof proactivaCompany.feedbackDataSharingConsentAt === "string"
          ? proactivaCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(proactivaCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(proactivaCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: proactivaSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(proactivaAgents[slug]) ? proactivaAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(proactivaProjects[slug]) ? proactivaProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(proactivaTasks[slug]) ? proactivaTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(proactivaRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(proactivaRoutines[slug]) ? proactivaRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  const strictSecretsMode = process.env.PROACTIVA_SECRETS_STRICT_MODE === "true";
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeProactivaSkillSyncPreference({ ...adapterConfig }, desiredSkills);
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".proactiva.yaml" ||
          relative === ".proactiva.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const proactivaAgentsOut: Record<string, Record<string, unknown>> = {};
    const proactivaProjectsOut: Record<string, Record<string, unknown>> = {};
    const proactivaTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const proactivaRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readProactivaSkillSyncPreference(
        ...
        proactivaAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      proactivaProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      proactivaTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      proactivaRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const proactivaExtensionPath = ".proactiva.yaml";
    const proactivaAgents = Object.fromEntries(
      Object.entries(proactivaAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const proactivaProjects = Object.fromEntries(
      Object.entries(proactivaProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const proactivaTasks = Object.fromEntries(
      Object.entries(proactivaTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const proactivaRoutines = Object.fromEntries(
      Object.entries(proactivaRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[proactivaExtensionPath] = buildYamlFile(
      {
        schema: "proactiva/v1",
        ...
        agents: Object.keys(proactivaAgents).length > 0 ? proactivaAgents : undefined,
        projects: Object.keys(proactivaProjects).length > 0 ? proactivaProjects : undefined,
        tasks: Object.keys(proactivaTasks).length > 0 ? proactivaTasks : undefined,
        routines: Object.keys(proactivaRoutines).length > 0 ? proactivaRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, proactivaExtensionPath);
