...
describe("opencode local skill sync", () => {
  ...
  it("reports configured Proactiva skills and installs them into the shared Claude/OpenCode skills home", async () => {
    const home = await makeTempDir("proactiva-opencode-skill-sync-");
    ...
    const after = await syncOpenCodeSkills(ctx, [proactivaKey]);
    expect(after.entries.find((entry) => entry.key === proactivaKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".claude", "skills", "proactiva"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled Proactiva skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("proactiva-opencode-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncOpenCodeSkills(configuredCtx, [proactivaKey]);
