...
vi.mock("@proactiva/shared/telemetry", async () => {
  const actual = await vi.importActual<typeof import("@proactiva/shared/telemetry")>(
    "@proactiva/shared/telemetry",
  );
  return {
    ...actual,
...
describeEmbeddedPostgres("heartbeat issue graph liveness escalation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("proactiva-heartbeat-issue-liveness-");
