...
import type { ProactivaPluginManifestV1 } from "@proactiva/shared";
import {
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("proactiva-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["proactiva.dbtest", "proactiva.escape"]) {
  ...
  async function createPluginPackage(manifest: ProactivaPluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "proactiva-plugin-package-"));
  ...
  async function installPluginRecord(manifest: ProactivaPluginManifestV1) {
  ...
  function manifest(pluginKey = "proactiva.dbtest"): ProactivaPluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("proactiva.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
