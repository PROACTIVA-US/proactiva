...
This guide describes the current, implemented way to create a Proactiva plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Proactiva app.
...
```bash
pnpm --filter @proactiva/create-proactiva-plugin build
node packages/plugins/create-proactiva-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Proactiva repo:

```bash
pnpm --filter @proactiva/create-proactiva-plugin build
node packages/plugins/create-proactiva-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/proactiva/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@proactiva/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@proactiva/plugin-sdk` from the local Proactiva checkout into a `.proactiva-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Proactiva from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@proactiva/plugin-sdk/ui`
