...
This document explains how Proactiva interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

Proactiva separates four concepts that are easy to blur together:
...
This is a hard invariant. Proactiva is single-assignee by design.

## 3. Status Semantics

Proactiva issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, Proactiva may still need a wake path to ensure the assignee actually sees it
...
- Proactiva can wake the assignee
- Proactiva can track runs linked to the issue
- Proactiva can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- Proactiva can track the ownership and status
- Proactiva cannot rely on heartbeat/run semantics to keep them moving
...
Proactiva already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.

## 6. Parent/Sub-Issue vs Blockers

Proactiva uses two different relationships for different jobs.
...
Blocked issues should stay idle while blockers remain unresolved. Proactiva should not create a queued heartbeat run for that issue until the final blocker is done and the `issue_blockers_resolved` wake can start real work.

If a parent is truly waiting on a child, model that with blockers. Do not rely on the parent/child relationship alone.

## 7. Consistent Execution Path Rules

For agent-assigned, non-terminal, actionable issues, Proactiva should not leave work in a state where nobody is working it and nothing will wake it.
...
Proactiva now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, Proactiva queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, Proactiva moves the issue to `blocked` and posts a visible comment
...
- Proactiva queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, Proactiva moves the issue to `blocked` and posts a visible comment
...
On startup and on the periodic recovery loop, Proactiva now does three things in sequence:
...
That is the execution contract Proactiva should present to operators.
