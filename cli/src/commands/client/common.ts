...
import { ApiRequestError, ProactivaApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Proactiva config file")
    .option("-d, --data-dir <path>", "Proactiva data directory root (isolates state from ~/.proactiva)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Proactiva API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.PROACTIVA_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.PROACTIVA_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.PROACTIVA_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set PROACTIVA_COMPANY_ID, or set context profile companyId via `proactiva context set`.",
    );
  }

  const api = new ProactivaApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.PROACTIVA_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.PROACTIVA_SERVER_PORT || "");
