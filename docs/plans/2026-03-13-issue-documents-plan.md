...
Add first-class **documents** to Proactiva as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Proactiva workflow/docs.
...
Update the Proactiva agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/proactiva/SKILL.md`
