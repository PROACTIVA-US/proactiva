...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.PROACTIVA_HOME = path.join(root, "home");
    process.env.PROACTIVA_INSTANCE_ID = "telemetry-test";
