...
type ProactivaSprite = (typeof PROACTIVA_SPRITES)[number];
...
function spriteSize(sprite: ProactivaSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = PROACTIVA_SPRITES[(baseCol + baseRow) % PROACTIVA_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = PROACTIVA_SPRITES[Math.floor(Math.random() * PROACTIVA_SPRITES.length)]!;
