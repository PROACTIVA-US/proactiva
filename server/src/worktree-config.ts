...
import type { ProactivaConfig } from "@proactiva/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Proactiva worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.PROACTIVA_IN_WORKTREE !== "true") return null;

  const configPath = resolveProactivaConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.PROACTIVA_WORKTREE_NAME) ??
    nonEmpty(env.PROACTIVA_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.PROACTIVA_INSTANCE_ID) ??
    nonEmpty(env.PROACTIVA_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.PROACTIVA_HOME) ??
      nonEmpty(env.PROACTIVA_HOME) ??
      nonEmpty(env.PROACTIVA_WORKTREES_DIR) ??
      "~/.proactiva-worktrees",
  );
...
function writeConfigFile(configPath: string, config: ProactivaConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".proactiva", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as ProactivaConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: ProactivaConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: ProactivaConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.PROACTIVA_HOME = context.homeDir;
  process.env.PROACTIVA_INSTANCE_ID = context.instanceId;
  process.env.PROACTIVA_CONFIG = context.configPath;
  process.env.PROACTIVA_CONTEXT = context.contextPath;
  process.env.PROACTIVA_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as ProactivaConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    PROACTIVA_HOME: context.homeDir,
    PROACTIVA_INSTANCE_ID: context.instanceId,
    PROACTIVA_CONFIG: context.configPath,
    PROACTIVA_CONTEXT: context.contextPath,
    PROACTIVA_IN_WORKTREE: "true",
    PROACTIVA_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: ProactivaConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as ProactivaConfig;
  } catch {
    return;
  }
