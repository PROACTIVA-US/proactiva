...
import { trackAgentFirstHeartbeat } from "@proactiva/shared/telemetry";
...
import { extractSkillMentionIds } from "@proactiva/shared";
...
const DEFERRED_WAKE_CONTEXT_KEY = "_proactivaWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const PROACTIVA_WAKE_PAYLOAD_KEY = "proactivaWake";
const PROACTIVA_HARNESS_CHECKOUT_KEY = "proactivaHarnessCheckedOut";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__proactiva_repo_only__";
...
export function applyRunScopedMentionedSkillKeys(
  ...
  const existingPreference = readProactivaSkillSyncPreference(config);
  return writeProactivaSkillSyncPreference(config, [
...
export function compactRunLogChunk(chunk: string, maxChars = MAX_PERSISTED_LOG_CHUNK_CHARS) {
  ...
  const marker = `\n[proactiva truncated run log chunk: omitted ${omittedChars} chars]\n`;
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[PROACTIVA_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[PROACTIVA_WAKE_PAYLOAD_KEY];
  }
  return merged;
}

async function buildProactivaWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  continuationSummary?:
  ...
  return {
    ...
    livenessContinuation: readNonEmptyString(input.contextSnapshot.livenessContinuationState) ||
      ...
      : null,
    checkedOutByHarness: input.contextSnapshot[PROACTIVA_HARNESS_CHECKOUT_KEY] === true,
...
export function heartbeatService(db: Db) {
    ...
    for (const candidate of candidates) {
      ...
      await issuesSvc.addComment(
        ...
        [
          ...
          "",
          `Proactiva found this issue is blocking ${blockingLinks} but had no assignee, so no heartbeat could pick it up.`,
          "",
    ...
    for (const issue of candidates) {
      ...
      if (issue.status === "todo") {
        ...
        if (didAutomaticRecoveryFail(latestRun, "assignment_recovery")) {
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            latestRun,
            comment:
              "Proactiva automatically retried dispatch for this assigned `todo` issue after a lost wake/run, " +
      ...
      if (didAutomaticRecoveryFail(latestRun, "issue_continuation_needed")) {
        ...
        const updated = await escalateStrandedAssignedIssue({
          ...
          latestRun,
          comment:
            "Proactiva automatically retried continuation for this assigned `in_progress` issue after its live " +
  ...
  function buildLivenessEscalationDescription(finding: IssueLivenessFinding) {
    return [
      "Proactiva detected a harness-level issue graph liveness incident.",
      "",
  ...
  function buildLivenessOriginalIssueComment(finding: IssueLivenessFinding, escalation: typeof issues.$inferSelect) {
    return [
      "Proactiva detected a harness-level liveness incident in this issue's dependency graph.",
      "",
  ...
  async function executeRun(runId: string) {
    ...
    if (
      ...
      try {
        ...
        context[PROACTIVA_HARNESS_CHECKOUT_KEY] = true;
      } catch (error) {
        if (!isCheckoutConflictError(error)) throw error;
        context[PROACTIVA_HARNESS_CHECKOUT_KEY] = false;
      }
    ...
    if (continuationSummary) {
      context.proactivaContinuationSummary = {
        key: continuationSummary.key,
        title: continuationSummary.title,
      ...
      delete context.proactivaContinuationSummary;
    }
    const proactivaWakePayload = await buildProactivaWakePayload({
      db,
      companyId: agent.companyId,
      contextSnapshot: context,
      continuationSummary,
    ...
    if (proactivaWakePayload) {
      context[PROACTIVA_WAKE_PAYLOAD_KEY] = proactivaWakePayload;
    } else {
      delete context[PROACTIVA_WAKE_PAYLOAD_KEY];
    }
    ...
    const runtimeConfig = {
      ...
      proactivaRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.proactivaWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.proactivaRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.proactivaRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.proactivaSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.proactivaSessionRotationReason = sessionCompaction.reason;
      context.proactivaPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.proactivaSessionHandoffMarkdown;
      delete context.proactivaSessionRotationReason;
      delete context.proactivaPreviousSessionId;
    }
    ...
    try {
      ...
      if (runScopedMentionedSkillKeys.length > 0) {
        await onLog(
          "stdout",
          `[proactiva] Enabled run-scoped skills from issue mentions: ${runScopedMentionedSkillKeys.join(", ")}\n`,
        );
      }
      ...
      if (runtimeServices.length > 0) {
        context.proactivaRuntimeServices = runtimeServices;
        context.proactivaRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[proactiva] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected PROACTIVA_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.proactivaRuntimeServices = combinedRuntimeServices;
        context.proactivaRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[proactiva] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
      ...
      if (finalizedRun) {
        ...
        if (issueId && outcome === "succeeded") {
          try {
            ...
            await onLog(
              "stderr",
              `[proactiva] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
  ...
  function buildImmediateExecutionPathRecoveryComment(input: {
    ...
    if (input.status === "todo") {
      return (
        "Proactiva automatically retried dispatch for this assigned `todo` issue during terminal run recovery, " +
        `but it still has no live execution path.${failureSummary ?? ""} ` +
        "Moving it to `blocked` so it is visible for intervention."
      );
    }
    ...
    return (
      "Proactiva automatically retried continuation for this assigned `in_progress` issue during terminal run " +
