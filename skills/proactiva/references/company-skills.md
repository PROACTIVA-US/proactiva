...
```sh
curl -sS -X POST "$PROACTIVA_API_URL/api/companies/$PROACTIVA_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$PROACTIVA_API_URL/api/companies/$PROACTIVA_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $PROACTIVA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$PROACTIVA_API_URL/api/companies/$PROACTIVA_COMPANY_ID/skills" \
  -H "Authorization: Bearer $PROACTIVA_API_KEY"
```
...
```sh
curl -sS "$PROACTIVA_API_URL/api/companies/$PROACTIVA_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $PROACTIVA_API_KEY"

curl -sS "$PROACTIVA_API_URL/api/companies/$PROACTIVA_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $PROACTIVA_API_KEY"
```
...
```sh
curl -sS -X POST "$PROACTIVA_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $PROACTIVA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$PROACTIVA_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $PROACTIVA_API_KEY"
```
...
```sh
curl -sS -X POST "$PROACTIVA_API_URL/api/companies/$PROACTIVA_COMPANY_ID/agents" \
  -H "Authorization: Bearer $PROACTIVA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in Proactiva runtime skills are still added automatically when required by the adapter.
