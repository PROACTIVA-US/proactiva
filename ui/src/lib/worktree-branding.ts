...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("proactiva-worktree-enabled") !== "true") return null;

  const name = readMetaContent("proactiva-worktree-name");
  const color = normalizeHexColor(readMetaContent("proactiva-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("proactiva-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
