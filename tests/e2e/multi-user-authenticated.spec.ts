...
const BASE = process.env.PROACTIVA_E2E_BASE_URL ?? "http://127.0.0.1:3105";
const DATA_DIR = process.env.PROACTIVA_E2E_DATA_DIR ?? process.env.PROACTIVA_HOME;
const CONFIG_PATH = process.env.PROACTIVA_E2E_CONFIG_PATH ?? path.resolve(process.cwd(), ".proactiva/config.json");
const BOOTSTRAP_SCRIPT_PATH = path.resolve(process.cwd(), "packages/db/scripts/create-auth-bootstrap-invite.ts");
const OWNER_PASSWORD = "proactiva-owner-password";
const INVITED_PASSWORD = "proactiva-invited-password";
...
const ownerUser: HumanUser = {
  name: "Owner User",
  email: `owner-${runId}@proactiva.local`,
  password: OWNER_PASSWORD,
};
const invitedUser: HumanUser = {
  name: "Invited User",
  email: `invitee-${runId}@proactiva.local`,
  password: INVITED_PASSWORD,
};

function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("PROACTIVA_E2E_DATA_DIR or PROACTIVA_HOME is required for authenticated bootstrap tests");
  }
...
async function signUp(page: Page, user: HumanUser) {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to Proactiva" })).toBeVisible();
  await page.getByRole("button", { name: "Create one" }).click();
  await expect(page.getByRole("heading", { name: "Create your Proactiva account" })).toBeVisible();
...
async function acceptBootstrapInvite(page: Page, inviteUrl: string) {
  ...
  await expect(page.getByRole("heading", { name: "Bootstrap your Proactiva instance" })).toBeVisible();
