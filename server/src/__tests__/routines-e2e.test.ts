...
describeEmbeddedPostgres("routine routes end-to-end", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("proactiva-routines-e2e-");
  ...
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@proactiva/shared/telemetry");
  ...
  it("runs routines with variable inputs and interpolates the execution issue description", async () => {
    ...
    expect(issue?.description).toBe("Review proactiva for high bugs");
  });
