...
} from "@proactiva/plugin-sdk";
import type { CreateIssueThreadInteraction, IssueDocumentSummary } from "@proactiva/shared";
 ...
 *
 * This implementation delegates to the core Proactiva domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@proactiva/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
