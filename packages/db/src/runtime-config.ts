...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "proactiva-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveProactivaHomeDir(), "instances", resolveProactivaInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".proactiva", CONFIG_BASENAME);
...
function resolveProactivaConfigPath(): string {
  if (process.env.PROACTIVA_CONFIG?.trim()) {
    return path.resolve(process.env.PROACTIVA_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveProactivaEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveProactivaEnvPath(configPath);
