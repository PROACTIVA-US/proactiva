...
Proactiva should not replace its workspace runtime service model with VS Code tasks.
...
- Proactiva owns long-running workspace services and their desired state
- Proactiva shows operators exactly which named thing they are starting or stopping
- Proactiva distinguishes long-running services from one-shot jobs
...
This is directionally correct for Proactiva because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Proactiva does not yet leverage existing workspace metadata.

## Recommendation

Keep Proactiva runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Proactiva.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Proactiva service or job semantics should become runnable in Proactiva.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Proactiva runtime services.
...
That creates a bad fit if Proactiva tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Proactiva's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Proactiva should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Proactiva can resolve
...
- selected variable substitution when Proactiva can resolve it safely from workspace context
- mapping task metadata into Proactiva readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Proactiva cannot execute directly
...
Proactiva needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Proactiva metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Proactiva
...
For a workspace with `cwd`, Proactiva should look for `.vscode/tasks.json`.
...
Proactiva should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Proactiva metadata marks it as a service, or
...
Proactiva-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Proactiva would import and why.
...
Goal:
run supported imported service commands through the current Proactiva supervisor.
...
Success condition:
imported service commands behave like native Proactiva services once adopted.
...
- removal of Proactiva's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Proactiva to VS Code and making the model worse for non-VS-Code repos
...
Proactiva should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Proactiva workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
