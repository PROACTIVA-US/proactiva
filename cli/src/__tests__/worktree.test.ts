...
function buildSourceConfig(): ProactivaConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("proactiva-pr-432")).toBe(
      path.resolve(os.homedir(), "proactiva-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("proactiva/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://proactiva.example", 3110)).toBe("https://proactiva.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/proactiva-feature",
      homeDir: "/tmp/proactiva-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/proactiva-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/proactiva-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.PROACTIVA_HOME).toBe(path.resolve("/tmp/proactiva-worktrees"));
    expect(env.PROACTIVA_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.PROACTIVA_IN_WORKTREE).toBe("true");
    expect(env.PROACTIVA_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.PROACTIVA_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export PROACTIVA_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  itEmbeddedPostgres("quarantines copied live execution state in seeded worktree databases", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("proactiva-worktree-quarantine-");
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-secrets-"));
    const originalInlineMasterKey = process.env.PROACTIVA_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.PROACTIVA_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.PROACTIVA_SECRETS_MASTER_KEY;
      delete process.env.PROACTIVA_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.PROACTIVA_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.PROACTIVA_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.PROACTIVA_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          PROACTIVA_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.PROACTIVA_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.PROACTIVA_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".proactiva-worktrees"),
      });

      const envPath = path.join(repoRoot, ".proactiva", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("PROACTIVA_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("PROACTIVA_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/PROACTIVA_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.PROACTIVA_AGENT_JWT_SECRET;
      } else {
        process.env.PROACTIVA_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-auth-seed-"));
      ...
      const worktreeHome = path.join(tempRoot, ".proactiva-worktrees");
      const originalCwd = process.cwd();
      const sourceDb = await startEmbeddedPostgresTestDatabase("proactiva-worktree-auth-source-");
      ...
      try {
        ...
        const targetConfig = JSON.parse(
          fs.readFileSync(path.join(worktreeRoot, ".proactiva", "config.json"), "utf8"),
        ) as ProactivaConfig;
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".proactiva-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".proactiva", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Proactiva config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".proactiva", "config.json");
    const originalCwd = process.cwd();
    const originalProactivaConfig = process.env.PROACTIVA_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalProactivaConfig === undefined) {
        delete process.env.PROACTIVA_CONFIG;
      } else {
        process.env.PROACTIVA_CONFIG = originalProactivaConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".proactiva", "config.json");
    const envPath = path.join(worktreeRoot, ".proactiva", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        envPath,
        [
          "PROACTIVA_HOME=/tmp/proactiva-worktrees",
          "PROACTIVA_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".proactiva", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".proactiva", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Proactiva instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".proactiva-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalProactivaConfig = process.env.PROACTIVA_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `PROACTIVA_HOME=${homeDir}`,
          `PROACTIVA_INSTANCE_ID=${currentInstanceId}`,
          "PROACTIVA_WORKTREE_NAME=existing-name",
          "PROACTIVA_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`PROACTIVA_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("PROACTIVA_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("PROACTIVA_WORKTREE_COLOR=\"#112233\"");
    } finally {
      process.chdir(originalCwd);
      if (originalProactivaConfig === undefined) {
        delete process.env.PROACTIVA_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".proactiva-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `PROACTIVA_HOME=${homeDir}\nPROACTIVA_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`PROACTIVA_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/proactiva",
        targetRepoRoot: "/Users/example/proactiva-pr-432",
        workspaceCwd: "/Users/example/proactiva",
      }),
    ).toBe("/Users/example/proactiva-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/proactiva",
        targetRepoRoot: "/Users/example/proactiva-pr-432",
        workspaceCwd: "/Users/example/proactiva/packages/db",
      }),
    ).toBe("/Users/example/proactiva-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "proactiva-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("proactiva-make-test", {
        seed: false,
        home: path.join(tempRoot, ".proactiva-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".proactiva", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".proactiva", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".proactiva", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".proactiva", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when Proactiva metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".proactiva", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".proactiva-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "proactiva-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".proactiva-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".proactiva", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".proactiva", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".proactiva", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("proactiva-worktree-routines-");
    const db = createDb(tempDb.connectionString);
    const companyId = randomUUID();
