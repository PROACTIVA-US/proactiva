...
if (tailscaleAuth) {
  ...
  env.HOST = "0.0.0.0";
  console.log("[proactiva] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
...
async function maybePreflightMigrations(options = {}) {
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[proactiva] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
