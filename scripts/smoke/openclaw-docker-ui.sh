...
PROACTIVA_HOST_PORT="${PROACTIVA_HOST_PORT:-3100}"
PROACTIVA_HOST_FROM_CONTAINER="${PROACTIVA_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.proactiva-openclaw.override.yml"
...
detect_proactiva_base_url() {
  ...
  for candidate in "$PROACTIVA_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${PROACTIVA_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${PROACTIVA_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
proactiva_base_url="$(detect_proactiva_base_url || true)"
...
Proactiva URL for OpenClaw container:
EOF
  if [[ -n "$proactiva_base_url" ]]; then
    cat <<EOF
  $proactiva_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${PROACTIVA_HOST_PORT}
  (Do not use http://127.0.0.1:${PROACTIVA_HOST_PORT} inside the container.)
  If Proactiva rejects the host, run on host machine:
    pnpm proactiva allowed-hostname host.docker.internal
  Then restart Proactiva and re-run this script.
EOF
  fi
  cat <<EOF
