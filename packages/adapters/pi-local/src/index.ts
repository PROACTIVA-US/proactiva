...
Use when:
- You want Proactiva to run Pi (the AI coding agent) locally as the agent runtime
...
- Proactiva requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/proactivas/ and resumed with --session.
